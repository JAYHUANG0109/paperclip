...
describe("buildPaperclipTaskMarkdown", () => {
  ...
  // Filing a task without a title gives it a server-derived placeholder. The
  // only thing that turns that placeholder into a real title is this directive
  // reaching the agent, so assert it is actually emitted.
  it("tells the agent to replace a server-derived title", () => {
    const markdown = buildPaperclipTaskMarkdown({
      issue: {
        ...
        workMode: "standard",
        description: "Fix the login redirect. It drops the return path.",
      },
    });

    expect(markdown).toContain("Title status: auto-generated placeholder");
    expect(markdown).toContain("Retitle directive:");
    expect(markdown).toContain("PATCH /api/issues/{issueId}");
  });

  // A title someone actually chose must never be rewritten out from under them.
  it("says nothing about retitling when a person wrote the title", () => {
    for (const titleAutoGenerated of [false, undefined]) {
      const markdown = buildPaperclipTaskMarkdown({
        issue: {
          id: "issue-1",
          identifier: "PAP-9001",
          title: "Deliberately chosen title",
          titleAutoGenerated,
          workMode: "standard",
      ...
      expect(markdown).not.toContain("Retitle directive:");
      expect(markdown).not.toContain("auto-generated placeholder");
    }
  });
});
