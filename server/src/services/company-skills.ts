...
export function companySkillService(db: Db) {
  ...
  async function detachSkillFromAllAgents(companyId: string, skillKey: string): Promise<number> {
    const skills = await listReferenceTargets(companyId);
    const agentRows = await agents.list(companyId, { includeTerminated: true });
    let changed = 0;
    await db.transaction(async (tx) => {
      for (const agent of agentRows) {
        const adapterConfig = agent.adapterConfig as Record<string, unknown>;
        const desiredEntries = resolveDesiredSkillEntries(skills, adapterConfig);
        if (!desiredEntries.some((entry) => entry.key === skillKey)) continue;
        ...
        await tx
          ...
          .where(and(eq(agentsTable.companyId, companyId), eq(agentsTable.id, agent.id)));
        changed += 1;
      }
    });
    return changed;
  ...
  async function deleteSkill(
    ...
    if (usedByAgents.length > 0) {
      ...
      // Force: detach from every agent first, then delete.
      await detachSkillFromAllAgents(companyId, skill.key);
    }
