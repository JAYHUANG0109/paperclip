...
import { deliverOutputToUserDrive } from "../services/google-drive.js";
...
// GENERIC_ATTACHMENT_CONTENT_TYPES now comes from the shared attachment module
// (upstream extracted it); the fork's local copy was dropped to avoid a
...
export function issueRoutes(
  ...
  router.post("/companies/:companyId/issues", applyCreateIssueStatusDefault, validate(createIssueSchema), async (req, res) => {
    ...
    // A blank title is the normal case for "type what you want, hit create".
    // Derive a placeholder so the task exists immediately, and record that it
    // is provisional so the assigned agent knows to replace it with a real
    // summary. Derived here rather than in the client so every caller of this
    // route - UI, plugins, agents - behaves the same way.
    const providedTitle = (createBody.title ?? "").trim();
    const titleIsProvisional = providedTitle.length === 0;
    createBody.title = titleIsProvisional
      ? deriveProvisionalIssueTitle(createBody.description)
      : providedTitle;
    ...
    const issue = await svc.create(companyId, {
      ...
      titleAutoGenerated: titleIsProvisional,
      id: issueId,
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    // `changes` arrives with upstream's authoritative PATCH receipts (627728bdd),
    // which this fork has not adopted yet. Read it through a shim so this commit is
    // independent: absent receipts mean "no field changes", the pre-receipt behaviour.
    const issueChanges =
      (issue as { changes?: Record<string, { from?: unknown; to?: unknown }> }).changes ?? {};
