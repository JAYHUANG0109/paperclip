...
export async function startServer(): Promise<StartedServer> {
  ...
  // Fork: createApp also returns the notification tool gateway, which the
  // heartbeat service below binds to.
  const { app, toolGateway } = await createApp(db as any, {
    ...
    wikiDistillEnabled: config.wikiDistillEnabled,
    wikiDistillCompanyId: config.wikiDistillCompanyId,
    wikiDistillIntervalMs: config.wikiDistillIntervalMs,
