...
function ImportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("companyImport.selectFilePreview")} />
    );
  }
...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("companyImport.selectZipPackage"));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("companyImport.noPackageFiles"));
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("companyImport.breadcrumbOrgChart"), href: "/org" },
      { label: t("companyImport.breadcrumbImport") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const previewMutation = useMutation({
    mutationFn: () => {
      ...
      if (!source) throw new Error(t("companyImport.noSourceConfigured"));
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toast.previewFailed"),
        body: err instanceof Error ? err.message : t("companyImport.toast.previewFailedBody"),
      });
    },
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t("companyImport.toast.importCompleteBody", { name: result.company.name, count: result.agents.length }),
      });
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("companyImport.toast.importFailed"),
        body: err instanceof Error ? err.message : t("companyImport.toast.importFailedBody"),
      });
    },
  });
  ...
  async function handleChooseLocalPackage(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      pushToast({
        tone: "error",
        title: t("companyImport.toast.packageReadFailed"),
        body: err instanceof Error ? err.message : t("companyImport.toast.packageReadFailedBody"),
      });
    }
  }
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          <h2 className="text-base font-semibold">{t("companyImport.importSource")}</h2>
          <p className="text-xs text-muted-foreground mt-1">
            {t("companyImport.importSourceDesc")}
          </p>
        </div>
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {t("companyImport.packageWithFiles", { name: localPackage.name, count: Object.keys(localPackage.files).length })}
                </span>
              )}
            </div>
            {!localPackage && (
        ...
        <Field label={t("companyImport.target")} hint={t("companyImport.targetHint")}>
          ...
          >
            <option value="new">{t("companyImport.createNewCompany")}</option>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {t("companyImport.filesSelected", { selected: selectedCount, total: totalFiles })}
              </span>
              {conflicts.length > 0 && (
          ...
          <div className="mx-5 mt-3 flex flex-wrap justify-end gap-2">
            ...
            >
              ...
              {importMutation.isPending
                ...
                : t("companyImport.importFiles", { count: selectedCount })}
            </Button>
          </div>
