...
const concurrencyPolicyDescriptionKeys: Record<string, string> = {
  coalesce_if_active: "routines.concurrency.coalesceIfActive",
  always_enqueue: "routines.concurrency.alwaysEnqueue",
  skip_if_active: "routines.concurrency.skipIfActive",
};
const catchUpPolicyDescriptionKeys: Record<string, string> = {
  skip_missed: "routines.catchUp.skipMissed",
  enqueue_missed_with_cap: "routines.catchUp.enqueueMissedWithCap",
};
...
export function Routines() {
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: routine.assigneeAgentId
          ? t("routines.toast.createdBody")
          : t("routines.toast.draftBody"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast.updateFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.updateFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        ...
        body: t("routines.toast.defaultAgentBody"),
        tone: "warn",
      });
      return;
    }
            ...
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("issues.view.sort")}>
                    <ArrowUpDown className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("issues.view.sort")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["updated", t("issues.sortField.updated")],
                      ["created", t("issues.sortField.created")],
                      ["lastRun", t("routines.sortLastRun")],
                      ["title", t("issues.sortField.title")],
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("issues.view.group")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("issues.view.group")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
                    {([
                      ["folder", t("routines.groupBy.folder", { defaultValue: "Folder" })],
        ...
        >
              ...
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  <InlineEntitySelector
                    ...
                    noneLabel={t("issues.props.noProject")}
                    searchPlaceholder={t("issues.props.searchProjects")}
              ...
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("routines.advancedSettingsHint")}</p>
                  </div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {concurrencyPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.concurrencyOption.${value}`)}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      ...
                      >
                        ...
                        <SelectContent>
                          {catchUpPolicies.map((value) => (
                            <SelectItem key={value} value={value}>{t(`routines.catchUpOption.${value}`)}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("common.creatingDots") : t("routines.createRoutine")}
              </Button>
