...
export function MyUserSecretsTab({ companyId }: { companyId: string }) {
  ...
  const clear = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("secrets.userSecrets.valueCleared"), tone: "info" });
    },
    onError: (err) =>
      pushToast({
        title: t("secrets.userSecrets.couldNotClearValue"),
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
      <div className="flex items-start gap-2 rounded-md border border-violet-500/30 bg-violet-500/5 px-4 py-3 text-xs text-violet-800 dark:text-violet-200">
        ...
        <p>
          {t("secrets.userSecrets.myBanner")}
          {missingCount > 0 ? (
            <span className="font-medium">
              {" "}
              {t("secrets.userSecrets.myBannerMissing", { count: missingCount })}
            </span>
          ) : null}
        </p>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {mySecretsQuery.isError ? (
          <div className="flex items-center gap-2 py-4 text-sm text-destructive">
            <AlertCircle className="h-4 w-4" /> {t("secrets.userSecrets.failedToLoadMine")}{" "}
          ...
          <EmptyState
            icon={KeyRound}
            message={t("secrets.userSecrets.myEmpty")}
          />
        ) : (
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {!disabledDefinition ? (
          <Button size="sm" variant={secret ? "outline" : "default"} onClick={onSet}>
            {secret ? t("secrets.userSecrets.update") : t("secrets.userSecrets.setValue")}
          </Button>
        ) : null}
        {secret ? (
          <Button
            ...
            onClick={onClear}
            disabled={clearing}
            title={t("secrets.userSecrets.clearMyValue")}
          >
