...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"]) {
  switch (status) {
    case "duplicate":
      return t("importVault.status.imported");
    case "conflict":
      return t("importVault.status.conflict");
    case "ready":
    default:
      return t("importVault.status.ready");
  }
}
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("importVault.rowStatus.created")}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("importVault.rowStatus.skipped")}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          <XCircle className="h-3 w-3" /> {t("importVault.rowStatus.failed")}
        </Badge>
      );
  }
}
...
function readableErrorMessage(error: unknown): string {
  if (error instanceof ApiError) {
    return error.message || t("importVault.requestFailed", { status: error.status });
  }
...
function validateDraftRow(
  ...
  if (!draft.name.trim()) return t("importVault.validation.nameRequired");
  if (draft.name.length > 160) return t("importVault.validation.nameTooLong");
  if (!draft.key.trim()) return t("importVault.validation.keyRequired");
  if (!KEY_PATTERN.test(draft.key)) {
    return t("importVault.validation.keyPattern");
  }
  if (draft.key.length > 120) return t("importVault.validation.keyTooLong");
  if (draft.description.length > 500) return t("importVault.validation.descTooLong");
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return t("importVault.validation.nameUsedSecret");
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return t("importVault.validation.keyUsedSecret");
    }
  }
  ...
  for (const other of otherDrafts) {
    ...
    if (other.name.trim().toLowerCase() === lowerName) {
      return t("importVault.validation.nameUsedBatch");
    }
    if (other.key.trim().toLowerCase() === lowerKey) {
      return t("importVault.validation.keyUsedBatch");
    }
  }
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      const vaultName =
        awsVaults.find((vault) => vault.id === vaultId)?.displayName ?? t("importVault.awsFallback");
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("importVault.toast.importFailed"),
          body: t("importVault.toast.noneImported", { vault: vaultName }),
          tone: "error",
        });
      } else {
        toast.pushToast({
          title: result.errorCount > 0 ? t("importVault.toast.completedWithErrors") : t("importVault.toast.complete"),
          body: t("importVault.toast.summary", { created: result.importedCount, skipped: result.skippedCount, failed: result.errorCount }),
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("importVault.toast.loadMoreFailed"),
          body: readableErrorMessage(error),
          tone: "error",
        });
      })
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(
        t("importVault.discardConfirm", { count: selection.size }),
      );
      if (!ok) return;
    }
    onOpenChange(false);
      ...
      >
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          ...
          <button
            ...
            onClick={() => handleClose()}
            aria-label={t("importVault.closeDialog")}
          >
            <X className="h-4 w-4" />
          </button>
        </header>
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "review" && (
              ...
              >
                {importMutation.isPending ? (
                  <>
                    <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("importVault.importing")}
                  </>
                ) : (
                  t("importVault.importN", { count: draftList.length })
                )}
              </Button>
            )}
            {step === "result" && (
...
function Stepper({ step }: { step: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: "select", label: t("importVault.step.select") },
    { id: "review", label: t("importVault.step.review") },
    { id: "result", label: t("importVault.step.result") },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("importVault.noVaultConfigured")}
          action={onManageVaults ? t("importVault.manageVaults") : undefined}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">{t("importVault.vault")}</label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger size="sm" className="text-xs" aria-label={t("importVault.selectVaultAria")}>
              <SelectValue placeholder={t("importVault.selectVaultPlaceholder")} />
            </SelectTrigger>
            <SelectContent>
              {awsVaults.map((vault) => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.isDefault && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px]">{t("importVault.defaultBadge")}</Badge>
                      )}
                      {vault.status === "warning" && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-amber-500 border-amber-500/40">{t("importVault.warningBadge")}</Badge>
                      )}
                      {blocked && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-muted-foreground">
                          {vault.status === "coming_soon" ? t("importVault.comingSoon") : vault.status}
                        </Badge>
                      )}
                    </span>
                  </SelectItem>
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("importVault.searchPlaceholder")}
            className="pl-7 pr-7 text-xs"
            aria-label={t("importVault.searchAria")}
        ...
        <Button
          ...
          aria-label={t("importVault.refreshAria")}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("importVault.selectedNotVisible", { selected: selection.size, hidden: selectedNotVisible })}
          </span>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            {showOnlySelected ? t("importVault.showAll") : t("importVault.showSelected")}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("importVault.selectAllLoaded", { count: selectableInLoaded.length })}
                    disabled={selectableInLoaded.length === 0}
                  />
                </th>
                <th className="px-2 py-2 text-left font-medium">{t("importVault.col.remoteName")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importVault.col.reference")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importVault.col.lastChanged")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importVault.col.suggestedName")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("importVault.col.state")}</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("importVault.selectRow", { name: candidate.remoteName })}
                      />
                    </td>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("importVault.loadedCount", { count: candidates.length })}
              {selectableInLoaded.length > 0 && (
                <span> · {t("importVault.selectableCount", { count: selectableInLoaded.length })}</span>
              )}
            </span>
            <Button
              variant="outline"
            ...
            >
              {pageLoading ? (
                ...
                t("importVault.loadMore", { count: PAGE_SIZE })
              )}
            </Button>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("importVault.err.denied")
            : isThrottling
              ? t("importVault.err.throttled")
              : t("importVault.err.couldNotLoad")}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("importVault.err.deniedDetail")
            : message}
        </div>
        <div className="mt-2 flex items-center gap-2">
          ...
          {isPermission && (
            ...
            >
              {t("importVault.iamReference")} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("importVault.noMatch", { query })}
      />
    );
  }
  return (
    <EmptyState
      icon={Database}
      message={t("importVault.noSecretsVisible")}
    />
  );
}
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("importVault.noneSelected")}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">{t("importVault.readyToImport", { count: ready })}</span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("importVault.needAttention", { count: blocked })}
          </span>
        )}
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importVault.paperclipName")}</span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importVault.key")}</span>
                      <Input
                        value={draft.key}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("importVault.descriptionOptional")}</span>
                      <Input
                ...
                <Button
                  ...
                  aria-label={t("importVault.removeRow", { name: draft.candidate.remoteName })}
                  className="h-7 w-7"
                  disabled={importing}
                >
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("importVault.toast.importFailed")
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("importVault.allImported", { count: result.importedCount })
        : t("importVault.toast.complete");
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">✓ {t("importVault.createdCount", { count: result.importedCount })}</span>
          <span>⊘ {t("importVault.skippedCount", { count: result.skippedCount })}</span>
          <span className="text-destructive">⨯ {t("importVault.failedCount", { count: result.errorCount })}</span>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup label={t("importVault.rowStatus.created")} rows={grouped.created} draftLookup={draftLookup} />
        )}
        {grouped.skipped.length > 0 && (
          <ResultGroup label={t("importVault.rowStatus.skipped")} rows={grouped.skipped} draftLookup={draftLookup} />
        )}
        {grouped.failed.length > 0 && (
          <ResultGroup label={t("importVault.rowStatus.failed")} rows={grouped.failed} draftLookup={draftLookup} />
        )}
      </div>
    </div>
  );
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("importVault.selectPrompt")
          : t("importVault.selectedCount", { count: totalSelected })}
      </div>
    );
  }
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("importVault.readyCount", { count: readyReviewCount })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            · {t("importVault.blockedCount", { count: blockedReviewCount })}
          </span>
        )}
      </div>
    );
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>{t("importVault.createdCount", { count: result.importedCount })}</span>
        <span>{t("importVault.skippedCount", { count: result.skippedCount })}</span>
        <span>{t("importVault.failedCount", { count: result.errorCount })}</span>
      </div>
    );
  }
  return null;
