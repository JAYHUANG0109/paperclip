    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={eventCount === 1 ? t("costs.eventsInRange_one", { count: eventCount }) : t("costs.eventsInRange_other", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costs.dateRangeTo")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.budgetPausedSubtitle", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs.budgetSpendOf", { spend: formatCents(spendData.summary.spendCents), budget: formatCents(spendData.summary.budgetCents) })
            ...
            <MetricTile
              ...
              subtitle={t("costs.financeNetSubtitle", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("costs.financeEvents")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("costs.financeEventsSubtitle", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("costs.tab.overview")}</TabsTrigger>
          <TabsTrigger value="budgets">{t("costs.tab.budgets")}</TabsTrigger>
          <TabsTrigger value="providers">{t("costs.tab.providers")}</TabsTrigger>
          <TabsTrigger value="billers">{t("costs.tab.billers")}</TabsTrigger>
          <TabsTrigger value="finance">{t("costs.tab.finance")}</TabsTrigger>
        </TabsList>
            ...
            <>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("costs.utilizationConsumed", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                ...
                <div className="space-y-4">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.financeTimelineEmpty")} />
                </div>
              </div>
            </>
          )}
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noBillableEventsPeriod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
