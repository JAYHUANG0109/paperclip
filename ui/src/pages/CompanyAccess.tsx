...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.company.breadcrumbSettings"), href: "/company/settings" },
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("companyAccess.toast.memberUpdated"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.updateMemberFailed"),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toast.joinApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.approveJoinFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("companyAccess.toast.joinRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.rejectJoinFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("companyAccess.toast.memberRemoved"),
        body:
          result.reassignedIssueCount > 0
            ? t("companyAccess.toast.issuesCleanedUp", { count: result.reassignedIssueCount })
            : undefined,
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("companyAccess.toast.removeMemberFailed"),
        body: error instanceof Error ? error.message : t("common.unknownError"),
        tone: "error",
      });
    },
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("companyAccess.subtitle", { name: selectedCompany?.name })}
        </p>
        <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
          {t("companyAccess.coreNote")}
        </div>
      </div>
      ...
      <section className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {t("companyAccess.pendingHumanJoinsDesc")}
                </p>
              </div>
            ...
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes} ${t("companyAccess.joinInvite")}${request.invite.humanRole ? ` • ${t("companyAccess.defaultRole", { role: request.invite.humanRole })}` : ""}`
                      : t("companyAccess.inviteUnavailable")
                  }
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("companyAccess.col.userAccount")}</div>
            <div>{t("companyAccess.col.role")}</div>
            <div>{t("companyAccess.col.status")}</div>
            <div className="text-right">{t("companyAccess.col.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("companyAccess.noMemberships")}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companyAccess.editMemberDesc", { name: editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId })}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="space-y-2 text-sm">
                  ...
                  >
                    <option value="active">{t("companyAccess.statusActive")}</option>
                    <option value="pending">{t("companyAccess.statusPending")}</option>
                    <option value="suspended">{t("companyAccess.statusSuspended")}</option>
                  </select>
                </label>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? t("common.saving") : t("companyAccess.saveMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("companyAccess.removeMemberDesc", { name: memberDisplayName(removingMember) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("companyAccess.checkingAssigned")
                    : t("companyAccess.openAssigned", { count: assignedIssues.length })}
                </div>
              </div>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {t("companyAccess.moreIssues", { count: assignedIssues.length - 6 })}
                      </div>
                    ) : null}
                  </div>
                </div>
...
export function CompanyAccessLegacyRoute() {
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("companyAccess.checkingExtensions")}</div>;
  }
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <div className="space-y-4 rounded-xl border border-border px-5 py-5">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            {t("companyAccess.advancedUnavailableDesc")}
          </p>
          {errorMessage ? (
            <p className="text-sm text-destructive">{t("companyAccess.pluginUnavailable", { error: errorMessage })}</p>
          ) : null}
        </div>
...
function memberDisplayName(member: CompanyMember | null) {
  if (!member) return t("companyAccess.thisMember");
