...
const STEPS: Array<{ key: CloudUpstreamStep; labelKey: string }> = [
  { key: "connect", labelKey: "cloudUpstream.step.connect" },
  { key: "scan", labelKey: "cloudUpstream.step.scan" },
  { key: "preview", labelKey: "cloudUpstream.step.preview" },
  { key: "push", labelKey: "cloudUpstream.step.push" },
  { key: "verify", labelKey: "cloudUpstream.step.verify" },
  { key: "activate", labelKey: "cloudUpstream.step.activate" },
];
const ACTIVATION_CATEGORIES: Array<{
  ...
  {
    key: "agents",
    get label() { return t("cloudUpstream.category.agents.label"); },
    get singular() { return t("cloudUpstream.category.agents.singular"); },
    get detail() { return t("cloudUpstream.category.agents.detail"); },
  },
  {
    key: "routines",
    get label() { return t("cloudUpstream.category.routines.label"); },
    get singular() { return t("cloudUpstream.category.routines.singular"); },
    get detail() { return t("cloudUpstream.category.routines.detail"); },
  },
  {
    key: "monitors",
    get label() { return t("cloudUpstream.category.monitors.label"); },
    get singular() { return t("cloudUpstream.category.monitors.singular"); },
    get detail() { return t("cloudUpstream.category.monitors.detail"); },
  },
];
...
export function CloudUpstream() {
  ...
  const finishMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setNotice(t("cloudUpstream.connectionApproved"));
      setActionError(null);
      await invalidateUpstreams();
      window.history.replaceState(null, "", settingsPath);
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.failedFinish")),
  });
  const {
  ...
  useEffect(() => {
    ...
    if (!pendingConnectionId) {
      setActionError(t("cloudUpstream.noPendingConnection"));
      return;
    }
  ...
  useEffect(() => {
    if (callbackError) {
      setActionError(t("cloudUpstream.connectionNotApproved", { error: callbackError }));
    }
  }, [callbackError]);
  ...
  const startMutation = useMutation({
    ...
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.failedStart")),
  });
  ...
  const runMutation = useMutation({
    ...
    onSuccess: async (run) => {
      ...
      setNotice(run.status === "succeeded"
        ? t("cloudUpstream.pushCompleted")
        : t("cloudUpstream.pushFailed"));
      setActionError(null);
      await invalidateUpstreams();
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.failedRunPush")),
  });
  const activationMutation = useMutation({
    ...
    onSuccess: async (run) => {
      setActiveRun(run);
      setNotice(t("cloudUpstream.checklistUpdated"));
      setActionError(null);
      await invalidateUpstreams();
    },
    onError: (error) => setActionError(error instanceof Error ? error.message : t("cloudUpstream.failedActivate")),
  });
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            {t("cloudUpstream.pushIntro", { company: selectedCompany.name })}
          </p>
        </div>
          ...
          {connection ? (
            <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {connection.target.product} · {connection.target.origin} · {t("cloudUpstream.tokenStatus", { status: connection.tokenStatus })}
                </div>
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("cloudUpstream.schemaChunk", { schema: connection.target.schemaMajor, chunk: formatBytes(connection.target.maxChunkBytes) })}
                </div>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Input
                ...
                aria-label={t("cloudUpstream.stackUrlAria")}
              />
      ...
      {preview ? (
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("cloudUpstream.preview")}</div>
            <Button
      ...
      {latestRun ? (
        <section className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("cloudUpstream.progressFinish")}</div>
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={() => downloadRunReport(latestRun)}>
                <FileJson className="h-4 w-4" />
                {t("cloudUpstream.downloadReport")}
              </Button>
          ...
          <div className="rounded-md border border-border px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {t("cloudUpstream.runShort", { id: latestRun.id.slice(0, 8) })} · {latestRun.completedAt ? t("cloudUpstream.completedAt", { date: formatDate(latestRun.completedAt) }) : t("cloudUpstream.inProgress")}
                </div>
              </div>
...
function PreviewProgressHint() {
  ...
  const message = elapsed < 15
    ? t("cloudUpstream.buildingManifest")
    : elapsed < 45
      ? t("cloudUpstream.buildingManifestElapsed", { seconds: elapsed })
      : t("cloudUpstream.buildingManifestSlow", { seconds: elapsed });
...
function ConflictTable({ conflicts }: { conflicts: CloudUpstreamPreview["conflicts"] }) {
  ...
  return (
    <div className="rounded-md border border-border px-4 py-3">
      <div className="mb-2 text-sm font-medium">{t("cloudUpstream.conflicts")}</div>
      {conflicts.length === 0 ? (
        <div className="text-sm text-muted-foreground">{t("cloudUpstream.noConflicts")}</div>
      ) : (
        <div className="divide-y divide-border">
...
function ActivationChecklist({
  ...
  return (
    <div className="rounded-md border border-border px-4 py-3">
      <div className="mb-2 text-sm font-medium">{t("cloudUpstream.activationChecklist")}</div>
      <div className="divide-y divide-border">
        {rows.map((row) => {
          ...
          return (
            <div key={row.key} className="grid gap-2 py-2 text-sm sm:grid-cols-[8rem_1fr_auto] sm:items-center">
              ...
              <div className="text-muted-foreground">
                {row.count === 0 ? t("cloudUpstream.zeroImportedInRun", { plural: row.pluralLabel }) : row.detail}
              </div>
              <div className="flex flex-wrap gap-2 sm:justify-end">
                ...
                >
                  ...
                  {activated ? t("cloudUpstream.activated") : t("cloudUpstream.activate")}
                </Button>
                <Button variant="ghost" size="sm" disabled={activated || isPending}>
                  {t("cloudUpstream.keepPaused")}
                </Button>
              </div>
            </div>
          );
...
export function buildActivationRows(run: CloudUpstreamRun) {
  ...
  return ACTIVATION_CATEGORIES.map((category) => {
    ...
    const pluralLabel = category.singular;
    return {
      ...category,
      count,
      pluralLabel,
      status,
      detail: `${t("cloudUpstream.importedPausedByDefault", { count, plural: pluralLabel })} ${category.detail}`,
      statusLabel: status === "activated"
        ? t("cloudUpstream.countActivated", { count })
        : count === 0
          ? t("cloudUpstream.zeroImported")
          : t("cloudUpstream.countPaused", { count }),
    };
  });
}
...
function previewErrorMessage(error: unknown): string {
  ...
  if (code === "payload_too_large" || code === "bad_request") {
    return t("cloudUpstream.previewTooLarge");
  }
  return code ?? t("cloudUpstream.failedPreview");
}
