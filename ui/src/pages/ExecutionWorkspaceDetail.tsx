...
function executionWorkspaceBaseTabItems(): OrderedExecutionWorkspaceTabItem[] {
  return [
    { value: "issues", label: t("nav.issues"), order: 10 },
    { value: "services", label: t("executionWorkspace.tabServices"), order: 20 },
    { value: "configuration", label: t("executionWorkspace.tabConfiguration"), order: 30 },
    { value: "runtime_logs", label: t("executionWorkspace.tabRuntimeLogs"), order: 40 },
    { value: "routines", label: t("nav.routines"), order: 60 },
  ];
}
...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : t("executionWorkspace.never");
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("projectWorkspace.errorJsonObject"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("projectWorkspace.errorInvalidJson"),
    };
  }
}
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return t("projectWorkspace.errorRepoUrlValid");
    }
  }
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("executionWorkspace.copied")}>
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("executionWorkspace.defaultAgentSet") : t("executionWorkspace.chooseAgentWhenRunning")}</span>
          <span>{t("executionWorkspace.lastRun", { time: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt) })}</span>
      ...
      >
        ...
        {isRunning ? t("executionWorkspace.running") : t("executionWorkspace.runNow")}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("executionWorkspace.routineStarted"),
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast.runFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("executionWorkspace.workspaceRoutines")}</CardTitle>
          <CardDescription>
            {t("executionWorkspace.workspaceRoutinesDesc")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("executionWorkspace.loadingRoutines")}</p>
          ) : error ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t("executionWorkspace.noWorkspaceRoutines")}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([...executionWorkspaceBaseTabItems(), ...workspacePluginTabItems]),
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("projectWorkspace.jobCompleted")
          : request.action === "stop"
            ? t("executionWorkspace.serviceStopped")
            : request.action === "restart"
              ? t("executionWorkspace.serviceRestarted")
              : t("projectWorkspace.serviceStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("projectWorkspace.failedControl"));
    },
  });
  ...
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("projectWorkspace.failedLoad")}
      </p>
    );
  }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
              {t("executionWorkspace.executionWorkspace")}
            </div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("executionWorkspace.noServicesStarted")
                : t("executionWorkspace.noCommandConfig")
            }
            jobEmptyMessage={t("executionWorkspace.noJobsConfigured")}
            disabledHint={
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.workspaceSettings")}</CardTitle>
                <CardDescription>
                  {t("executionWorkspace.workspaceSettingsDesc")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("executionWorkspaceClose.retryClose") : t("executionWorkspaceClose.closeWorkspace")}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.sectionGeneral")}</div>
                  <Field label={t("projectWorkspace.fieldName")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("executionWorkspace.namePlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.sectionSourceControl")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("executionWorkspace.fieldBranchName")} hint={t("executionWorkspace.fieldBranchHint")}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("executionWorkspace.fieldBaseRef")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("projectWorkspace.fieldRepoUrl")}>
                    <Input
                      value={form.repoUrl}
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.sectionPaths")}</div>
                  <Field label={t("executionWorkspace.fieldWorkingDir")}>
                    <Input
                      className="font-mono"
                      value={form.cwd}
                  ...
                  <Field label={t("executionWorkspace.fieldProviderRef")}>
                    <Input
                      className="font-mono"
                      value={form.providerRef}
                      onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                      placeholder={t("executionWorkspace.providerRefPlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.sectionLifecycle")}</div>
                  <Field label={t("executionWorkspace.fieldProvisionCommand")} hint={t("executionWorkspace.fieldProvisionHint")}>
                    <Textarea
                  ...
                  <Field label={t("executionWorkspace.fieldTeardownCommand")} hint={t("executionWorkspace.fieldTeardownHint")}>
                    <Textarea
                      className="min-h-20 font-mono"
                  ...
                  <Field label={t("projectWorkspace.fieldCleanupCommand")} hint={t("executionWorkspace.fieldCleanupHint")}>
                    <Textarea
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("executionWorkspace.sectionRuntimeConfig")}</div>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("executionWorkspace.runtimeOverrides")
                            : runtimeConfigSource === "project_workspace"
                              ? t("executionWorkspace.runtimeInheriting")
                              : t("executionWorkspace.runtimeNone")}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("projectWorkspace.advancedJson")}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("executionWorkspace.advancedJsonDesc")}
                    </p>
                    <div className="mt-3">
                      <Field label={t("projectWorkspace.commandsJson")} hint={t("executionWorkspace.commandsJsonHint")}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("executionWorkspace.inheritRuntimeConfig")}</label>
                        </div>
                        <Textarea
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.workspaceContext")}</CardTitle>
                <CardDescription>{t("executionWorkspace.workspaceContextDesc")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("projectWorkspace.detailProject")}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailProjectWorkspace")}>
              ...
              <DetailRow label={t("executionWorkspace.detailSourceIssue")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("executionWorkspace.detailDerivedFrom")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("projectWorkspace.detailWorkspaceId")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("executionWorkspace.concreteLocation")}</CardTitle>
                <CardDescription>{t("executionWorkspace.pathsAndRefs")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("executionWorkspace.detailWorkingDir")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("projectWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailProviderRef")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("projectWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("projectWorkspace.fieldRepoUrl")}>
                {workspace.repoUrl && isSafeExternalUrl(workspace.repoUrl) ? (
                  <div className="inline-flex max-w-full items-start gap-2">
                    ...
                    <CopyText text={workspace.repoUrl} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("executionWorkspace.copied")}>
                      <Copy className="h-3.5 w-3.5" />
                    </CopyText>
                  </div>
                ) : workspace.repoUrl ? (
              ...
              <DetailRow label={t("executionWorkspace.fieldBaseRef")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("projectWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailBranch")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("projectWorkspace.none")}
              </DetailRow>
              <DetailRow label={t("executionWorkspace.detailOpened")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspace.detailLastUsed")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("executionWorkspace.detailCleanup")}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("issues.props.notScheduled")}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("executionWorkspace.runtimeLogsTitle")}</CardTitle>
              <CardDescription>{t("executionWorkspace.recentOperations")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("executionWorkspace.loadingOperations")}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("executionWorkspace.failedLoadOperations")}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("executionWorkspace.noOperations")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("executionWorkspace.loadingPlugin")}</CardContent>
          </Card>
          ...
          <MissingPluginTabPlaceholder
            ...
            defaultTabLabel={t("executionWorkspace.backToIssues")}
          />
        ) : activeTab === "routines" ? (
