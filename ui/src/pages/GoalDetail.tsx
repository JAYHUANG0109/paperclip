...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("goals.detail.showProperties")}
    >
...
export function GoalDetail() {
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("goals.detail.noCompanySelected"));
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("goals.detail.breadcrumb"), href: "/goals" },
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("goals.detail.subGoalsTab", { count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("goals.detail.projectsTab", { count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("goals.detail.subGoal")}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goals.detail.noSubGoals")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("goals.detail.noLinkedProjects")}</p>
          ) : (
