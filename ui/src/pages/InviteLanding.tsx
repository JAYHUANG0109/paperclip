...
function mapInviteAuthFeedback(
  ...
  if (code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL") {
    return {
      tone: "info",
      message: t("inviteLanding.accountExists", { email: emailLabel }),
    };
  }
  ...
  if (authMode === "sign_up" && message === "Request failed: 422") {
    return {
      tone: "info",
      message: t("inviteLanding.accountMayExist", { email: emailLabel }),
    };
  }

  return {
    tone: "error",
    message: message ?? t("inviteLanding.authFailed"),
  };
}
...
export function InviteLandingPage() {
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isCheckingExistingMembership) {
        throw new Error(t("inviteLanding.checkingAccessRetry"));
      }
      if (isCurrentMember) {
        throw new Error(t("inviteLanding.alreadyBelongs"));
      }
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("inviteLanding.failedAccept"));
    },
  });
  ...
  const joinButtonLabel = useMemo(() => {
    ...
    return sessionQuery.data ? t("inviteLanding.acceptInvite") : t("common.continue");
        ...
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {inviteJoinRequestStatus === "rejected"
              ? t("inviteLanding.requestNotApproved")
              : t("inviteLanding.alreadyUsed")}
          </p>
        </div>
      </div>
    );
          ...
          <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex items-center gap-3">
              <InviteCompanyLogo
                ...
                className="h-12 w-12 border border-zinc-800 rounded-none"
              />
              <h1 className="text-lg font-semibold">{t("inviteLanding.youJoined")}</h1>
            </div>
            <div className="mt-4">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-2 text-2xl font-semibold">
                  {invite.inviteType === "bootstrap_ceo" ? t("inviteLanding.setUpPaperclip") : t("inviteLanding.joinCompany", { company: companyDisplayName })}
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t("inviteLanding.reviewSubmitAgent")
                    : requiresHumanAccount
                      ? t("inviteLanding.createAccountFirst")
                      : t("inviteLanding.accountReadyAccept")}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">{t("inviteLanding.requestedAccess")}</div>
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? t("inviteLanding.agentJoinRequest") : requestedHumanRole ?? t("inviteLanding.companyAccess")}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">{t("inviteLanding.inviteExpires")}</div>
            ...
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {t("inviteLanding.agentApprovalNote", { company: companyDisplayName })}
                  </p>
                </div>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.agentName")}</span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.adapterType")}</span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
                  ...
                  >
                    {joinAdapterOptions.map((type) => (
                      <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                        {getAdapterLabel(type)}{!ENABLED_INVITE_ADAPTERS.has(type) ? t("inviteLanding.comingSoonSuffix") : ""}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">{t("inviteLanding.capabilities")}</span>
                  <textarea
                    className={fieldClassName}
                    rows={4}
                    value={capabilities}
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                      ? t("inviteLanding.signUpHint", { company: companyDisplayName })
                      : t("inviteLanding.signInHint")}
                  </p>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({ tone: "error", message: t("inviteLanding.fillRequired") });
                      return;
                    }
                    authMutation.mutate();
                  }}
                ...
                >
                  {authMode === "sign_up" ? (
                    <label className="block text-sm">
                      <span className="mb-1 block text-zinc-400">{t("inviteLanding.name")}</span>
                      <input
                        name="name"
                        className={fieldClassName}
                        value={name}
                  ...
                  <label className="block text-sm">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.email")}</span>
                    <input
                      name="email"
                      type="email"
                      className={fieldClassName}
                  ...
                  <label className="block text-sm">
                    <span className="mb-1 block text-zinc-400">{t("inviteLanding.password")}</span>
                    <input
                      name="password"
                      type="password"
                      className={fieldClassName}
                  ...
                  >
                    {authMutation.isPending
                      ...
                      : authMode === "sign_in"
                        ? t("inviteLanding.signInContinue")
                        : t("inviteLanding.createAccountContinue")}
                  </Button>
                </form>
              ...
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {isCurrentMember
                      ? t("inviteLanding.alreadyInCompany")
                      : shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.completingAccess")
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("inviteLanding.acceptBootstrap")
                        : t("inviteLanding.acceptCompanyInvite")}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.grantingAccess", { company: companyDisplayName })
                      : isCurrentMember
                      ? t("inviteLanding.accountBelongs", { company: companyDisplayName })
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("inviteLanding.willFinishSetup")
                        : t("inviteLanding.willGrantAccess", { company: companyDisplayName })}
                  </p>
                </div>
                {error ? <p className="text-xs text-red-400">{error}</p> : null}
                {shouldAutoAcceptHumanInvite ? (
