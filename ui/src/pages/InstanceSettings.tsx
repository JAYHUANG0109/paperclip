...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("settings.instance.heartbeats.breadcrumb") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.heartbeats.failedUpdate"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("settings.instance.heartbeats.unknownError");
        throw new Error(
          failures.length === 1
            ? t("settings.instance.heartbeats.failedDisableOne", { detail })
            : t("settings.instance.heartbeats.failedDisableMany", { count: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.instance.heartbeats.failedDisableAll"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.instance.heartbeats.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("settings.instance.heartbeats.failedLoad")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.instance.heartbeats.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.instance.heartbeats.subtitle")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("settings.instance.heartbeats.active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("settings.instance.heartbeats.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? t("settings.instance.heartbeats.company") : t("settings.instance.heartbeats.companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("settings.instance.heartbeats.disableAllConfirm", { count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("settings.instance.heartbeats.disabling") : t("settings.instance.heartbeats.disableAll")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("settings.instance.heartbeats.emptyState")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("settings.instance.heartbeats.on") : t("settings.instance.heartbeats.off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("settings.instance.heartbeats.never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("settings.instance.heartbeats.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("settings.instance.heartbeats.disableTimer") : t("settings.instance.heartbeats.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
