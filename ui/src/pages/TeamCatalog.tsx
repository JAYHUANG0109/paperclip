...
function trustMeta(
  t: TranslateFn,
): Record<CatalogTeamTrustLevel, { label: string; tip: string; tone: string; Icon: typeof ShieldCheck }> {
  return {
    markdown_only: {
      label: t("teamCatalog.trust.markdownOnly.label", { defaultValue: "Markdown only" }),
      tip: t("teamCatalog.trust.markdownOnly.tip", { defaultValue: "Contains only markdown and references. No executable content." }),
      tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30",
      Icon: ShieldCheck,
    },
    assets: {
      label: t("teamCatalog.trust.assets.label", { defaultValue: "Assets" }),
      tip: t("teamCatalog.trust.assets.tip", { defaultValue: "Includes static assets (images, fixtures). No executable content." }),
      tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30",
      Icon: ShieldCheck,
    },
    scripts_executables: {
      label: t("teamCatalog.trust.scripts.label", { defaultValue: "Scripts" }),
      tip: t("teamCatalog.trust.scripts.tip", { defaultValue: "Includes executable scripts that were security-reviewed before bundling." }),
      tone: "text-amber-600 dark:text-amber-300 border-amber-500/30",
      Icon: AlertTriangle,
    },
    external_sources: {
      label: t("teamCatalog.trust.externalSources.label", { defaultValue: "External sources" }),
      tip: t("teamCatalog.trust.externalSources.tip", { defaultValue: "References external sources resolved at install time." }),
      tone: "text-amber-600 dark:text-amber-300 border-amber-500/30",
      Icon: AlertTriangle,
    },
  };
...
function compatMeta(t: TranslateFn): Record<CatalogTeamCompatibility, { label: string; tone: string }> {
  return {
    compatible: { label: t("teamCatalog.compat.compatible", { defaultValue: "Compatible" }), tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30" },
    unknown: { label: t("teamCatalog.compat.unknown", { defaultValue: "Unknown compat" }), tone: "text-muted-foreground border-border" },
    invalid: { label: t("teamCatalog.compat.invalid", { defaultValue: "Invalid" }), tone: "text-rose-600 dark:text-rose-300 border-rose-500/30" },
  };
}

function CompatChip({ compatibility }: { compatibility: CatalogTeamCompatibility }) {
  const { t } = useTranslation();
  const meta = compatMeta(t)[compatibility];
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[11px] font-medium",
...
function ProvenanceBadge({ team }: { team: CatalogTeam }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("teamCatalog.provenanceTip", { defaultValue: "Catalog package provenance" })}</TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-medium">
        ...
        {unsafe.length === 1
          ? t("teamCatalog.riskBanner_one", { defaultValue: "This team references {{count}} external source", count: unsafe.length })
          : t("teamCatalog.riskBanner_other", { defaultValue: "This team references {{count}} external sources", count: unsafe.length })}
      </div>
        ...
        {unsafe.map((s) => (
          <li key={`${s.type}:${s.ref}`} className="font-mono">
            ...
            <span className="not-italic font-sans opacity-80">
              ({sourceWarningCode(s) === "unsupported_in_ui" ? t("teamCatalog.source.unsupportedInBrowserInstall", { defaultValue: "unsupported in browser install" }) : t("teamCatalog.source.unpinned", { defaultValue: "unpinned" })})
            </span>
          </li>
        ))}
      </ul>
      ...
      <ul className="divide-y divide-border/60">
        {team.rootAgentSlugs.map((slug) => (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">{t("teamCatalog.rootAgent", { defaultValue: "root agent" })}</span>
          </li>
        ))}
        {members.map((slug) => (
        ...
        {team.agentSlugs.length === 0 && (
          <li className="px-3 py-2 text-xs text-muted-foreground">{t("teamCatalog.noAgentsInTeam", { defaultValue: "No agents in this team." })}</li>
        )}
      </ul>
    </div>
  );
        ...
        >
          ...
          {skill.resolved ? (
            <Badge variant="outline" className="text-[10px] text-emerald-600 dark:text-emerald-300 border-emerald-500/30">
              {t("teamCatalog.skill.resolved", { defaultValue: "resolved" })}
            </Badge>
          ) : (
            <Badge variant="outline" className="text-[10px] text-amber-600 dark:text-amber-300 border-amber-500/30">
              {t("teamCatalog.skill.external", { defaultValue: "external" })}
            </Badge>
          )}
        </li>
      ))}
        ...
        {inputs.map((input) => (
          ...
          >
            ...
            {input.requirement === "required" && (
              <Badge variant="outline" className="text-[10px]">{t("teamCatalog.required", { defaultValue: "required" })}</Badge>
            )}
          </li>
        ))}
      </ul>
...
export function ExternalSourcesList({ sources }: { sources: CatalogTeamSourceRef[] }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        {open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
        {t("teamCatalog.externalSourcesCount", { defaultValue: "External sources · {{count}}", count: external.length })}
      </button>
      {open && (
...
export function TeamDetailPane({
  ...
  const installButton = (
    ...
    >
      ...
      {isInstalled ? t("teamCatalog.reinstallLatest", { defaultValue: "Re-install latest" }) : t("teamCatalog.installTeam", { defaultValue: "Install team" })}
    </Button>
  );
        ...
        <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {isInstalled && !outOfDate && (
                <Badge variant="secondary" className="gap-1 text-[10px]">
                  <CheckCircle2 className="h-3 w-3" /> {t("teamCatalog.installed", { defaultValue: "Installed" })}
                </Badge>
              )}
              {outOfDate && (
                <Badge
                ...
                >
                  <ChevronUp className="h-3 w-3" /> {t("teamCatalog.updateAvailable", { defaultValue: "Update available" })}
                </Badge>
              )}
            </div>
          </div>
          {invalid ? (
            <Tooltip>
              ...
              <TooltipContent>{t("teamCatalog.cannotInstallInvalid", { defaultValue: "This team cannot be installed — the package manifest is invalid." })}</TooltipContent>
            </Tooltip>
          ) : !canInstall ? (
            <Tooltip>
              <TooltipTrigger asChild>
              ...
              <TooltipContent>{t("teamCatalog.requiresPermissions", { defaultValue: "Requires board operator or agent-create permissions." })}</TooltipContent>
            </Tooltip>
          ) : (
            installButton
          )}
...
function stepLabels(t: TranslateFn): Record<WizardStep, string> {
  return {
    target_manager: t("teamCatalog.step.targetManager", { defaultValue: "Target manager" }),
    source_policy: t("teamCatalog.step.sourcePolicy", { defaultValue: "Source policy" }),
    skill_plan: t("teamCatalog.step.prerequisiteSkills", { defaultValue: "Prerequisite skills" }),
    preview: t("teamCatalog.step.preview", { defaultValue: "Preview" }),
  };
}
...
export function useInstallTeamCatalogEntry({
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setPreviewError(error instanceof Error ? error.message : standaloneT("teamCatalog.failedToLoadPreview", { defaultValue: "Failed to load install preview." }));
    },
  });
  ...
  const installMutation = useMutation({
    ...
    onError: (error) => {
      setPhase("error");
      setApplyError(error instanceof Error ? error.message : standaloneT("teamCatalog.installFailedShort", { defaultValue: "Install failed." }));
    },
  });
...
function TeamInstallerDialog({
  ...
  const { t } = useTranslation();
  const STEP_LABELS = useMemo(() => stepLabels(t), [t]);
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setPreviewError(error instanceof Error ? error.message : t("teamCatalog.failedToLoadPreview", { defaultValue: "Failed to load install preview." }));
    },
  });
  ...
  const installMutation = useMutation({
    ...
    onError: (error) => {
      setPhase("error");
      setApplyError(error instanceof Error ? error.message : t("teamCatalog.installFailedShort", { defaultValue: "Install failed." }));
    },
  });
  ...
  const headerTitle = (
    <span className="flex items-center gap-2">
      ...
      {t("teamCatalog.installNamed", { defaultValue: "Install {{name}}", name: team.name })}
    </span>
  );
  const headerDescription =
    phase === "form" ? (
      <span className="flex items-center gap-2">
        <span>
          {t("teamCatalog.stepProgress", { defaultValue: "Step {{current}} of {{total}} · {{label}}", current: stepIndex + 1, total: totalSteps, label: STEP_LABELS[currentStep] })}
        </span>
            ...
            <div role="alert" className="flex items-start gap-2 rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2.5 text-sm text-rose-700 dark:text-rose-300">
              ...
              <div>
                ...
                <p className="mt-1 text-xs opacity-80">
                  {t("teamCatalog.installFailedHint", { defaultValue: "Partial state is not rolled back. Review the company activity log before retrying." })}
                </p>
              </div>
            </div>
          </div>
  ...
  const footer =
    phase === "form" ? (
      <div className="flex items-center justify-between gap-3">
        <div>
          {stepIndex > 0 ? (
            <Button variant="ghost" onClick={goBack}>{t("common.back", { defaultValue: "Back" })}</Button>
          ) : (
        ...
        <div className="flex items-center gap-3">
          {currentStep === "preview" && hasErrors && (
            <span className="text-xs text-rose-600 dark:text-rose-300">
              {blockedCount === 1
                ? t("teamCatalog.installBlockedErrors_one", { defaultValue: "Install blocked: {{count}} error", count: blockedCount })
                : t("teamCatalog.installBlockedErrors_other", { defaultValue: "Install blocked: {{count}} errors", count: blockedCount })}
            </span>
          )}
          {currentStep === "preview" && !hasErrors && missingRequiredSecretCount > 0 && (
            <span className="text-xs text-rose-600 dark:text-rose-300">
              {t("teamCatalog.requiredSecretsMissing", { defaultValue: "Required secrets missing: {{count}}", count: missingRequiredSecretCount })}
            </span>
          )}
          {currentStep === "preview" ? (
            needsScriptsConfirm && confirmScripts ? (
              <Button variant="destructive" onClick={submitInstall} disabled={installBlocked || previewMutation.isPending}>
                <AlertTriangle className="h-4 w-4" />
                {t("teamCatalog.confirmInstallExecutables", { defaultValue: "Confirm — install with executables" })}
              </Button>
            ) : (
              <Button onClick={submitInstall} disabled={installBlocked || previewMutation.isPending || !previewResult}>
                ...
                {needsScriptsConfirm ? t("teamCatalog.installWithExecutables", { defaultValue: "Install with executables" }) : t("teamCatalog.installTeam", { defaultValue: "Install team" })}
              </Button>
            )
          ) : (
            <Button onClick={goNext} disabled={!canContinue(currentStep)}>{t("common.continue", { defaultValue: "Continue" })}</Button>
          )}
        </div>
      </div>
    ) : phase === "error" ? (
      <div className="flex justify-end">
        <Button variant="ghost" onClick={onClose}>{t("common.close", { defaultValue: "Close" })}</Button>
      </div>
    ) : null;
...
export function StepTargetManager({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {t("teamCatalog.targetManagerHelp", { defaultValue: "This team's root agents need a manager in your company. Pick the agent who will become their parent. Internal team hierarchy is preserved." })}
      </div>
      ...
      {canBypassManager && (
        <label className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
          ...
          />
          {t("teamCatalog.useAsFullCompany", { defaultValue: "Use this team as a full-company package (no target manager)" })}
        </label>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div role="alert" className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-sm text-amber-700 dark:text-amber-300">
        {external.length === 1
          ? t("teamCatalog.sourcePolicyIntro_one", { defaultValue: "This team references {{count}} external source. Review each one and decide what to allow before continuing.", count: external.length })
          : t("teamCatalog.sourcePolicyIntro_other", { defaultValue: "This team references {{count}} external sources. Review each one and decide what to allow before continuing.", count: external.length })}
      </div>
            ...
            <li key={`${source.type}:${source.ref}`} className="flex items-center gap-2 px-3 py-2.5 text-sm">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {code === "ok" && t("teamCatalog.sourceCode.pinned", { defaultValue: "pinned" })}
                  {code === "unpinned" && t("teamCatalog.sourceCode.unpinnedReference", { defaultValue: "unpinned reference" })}
                  {code === "unsupported_in_ui" && t("teamCatalog.sourceCode.notInstallable", { defaultValue: "not installable from the browser" })}
                </p>
              </div>
              <Badge
                variant="outline"
      ...
      <div className="space-y-2.5 rounded-md border border-border p-3">
        <PolicyToggle
          label={t("teamCatalog.policy.external.label", { defaultValue: "Allow external sources" })}
          description={t("teamCatalog.policy.external.description", { defaultValue: "Resolve github/url skill and team sources at install time." })}
          checked={allowExternalSources}
          onChange={(v) => onChange("external", v)}
        />
        <PolicyToggle
          label={t("teamCatalog.policy.unpinned.label", { defaultValue: "Allow unpinned optional sources" })}
          description={t("teamCatalog.policy.unpinned.description", { defaultValue: "Permit optional sources that are not pinned to a ref or checksum." })}
          checked={allowUnpinnedOptionalSources}
          onChange={(v) => onChange("unpinned", v)}
        />
        <PolicyToggle
          label={t("teamCatalog.policy.localPath.label", { defaultValue: "Allow local-path sources" })}
          description={t("teamCatalog.policy.localPath.description", { defaultValue: "Required for local_path / agent_package sources. Development use only." })}
      ...
      {hasUnsupported && !allowLocalPathSources && (
        <p className="text-xs text-rose-600 dark:text-rose-300">
          {t("teamCatalog.localPathWarning", { defaultValue: "This team has local-path sources. Enable “Allow local-path sources” to continue, or install it from the CLI." })}
        </p>
      )}
    </div>
  );
...
function skillActionMeta(
  t: TranslateFn,
): Record<CatalogTeamSkillPreparation["action"], { label: string; tone: string }> {
  return {
    already_in_package: { label: t("teamCatalog.skillAction.alreadyInPackage", { defaultValue: "Bundled in package" }), tone: "text-emerald-600 dark:text-emerald-300 border-emerald-500/30" },
    catalog_install_required: { label: t("teamCatalog.skillAction.catalogInstall", { defaultValue: "Will install from catalog" }), tone: "text-blue-600 dark:text-blue-300 border-blue-500/30" },
    external_import_required: { label: t("teamCatalog.skillAction.externalImport", { defaultValue: "Will import from source" }), tone: "text-amber-600 dark:text-amber-300 border-amber-500/30" },
    blocked: { label: t("teamCatalog.skillAction.blocked", { defaultValue: "Blocked" }), tone: "text-rose-600 dark:text-rose-300 border-rose-500/30" },
  };
}
...
export function StepSkillPlan({
  ...
  const { t } = useTranslation();
  const SKILL_ACTION_META = skillActionMeta(t);
  ...
  return (
    <div className="space-y-4">
      <div role="alert" className="rounded-md border border-blue-500/30 bg-blue-500/10 px-3 py-2.5 text-sm text-blue-700 dark:text-blue-300">
        {t("teamCatalog.skillPlanIntro", { defaultValue: "Before agents are imported, the catalog resolves the skills they depend on. This is the resolution plan." })}
      </div>
      <ul className="divide-y divide-border rounded-md border border-border">
  ...
  if (loading && !result) {
    return (
      <div className="flex items-center gap-2 py-12 text-sm text-muted-foreground">
        <Loader2 className="h-4 w-4 animate-spin" /> {t("teamCatalog.preparingPreview", { defaultValue: "Preparing preview…" })}
      </div>
    );
  }
  if (error) {
    return (
      <div className="space-y-3">
        ...
        <Button variant="outline" onClick={onRetry}>
          <RotateCcw className="h-4 w-4" /> {t("common.retry", { defaultValue: "Retry" })}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {plan.agentPlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.agentsCount", { defaultValue: "Agents · {{count}}", count: plan.agentPlans.length })}>
      ...
      {plan.projectPlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.projectsCount", { defaultValue: "Projects · {{count}}", count: plan.projectPlans.length })}>
      ...
      {plan.issuePlans.length > 0 && (
        <PreviewSection title={t("teamCatalog.starterTasksCount", { defaultValue: "Starter tasks · {{count}}", count: plan.issuePlans.length })}>
      ...
      {manifestAgents.length > 0 && (
        <PreviewSection title={t("teamCatalog.adapterSelectionCount", { defaultValue: "Adapter selection · {{count}}", count: manifestAgents.length })}>
          ...
          <li className="px-3 py-1.5 text-[11px] text-muted-foreground">
            {t("teamCatalog.adapterNote", { defaultValue: "Each imported agent defaults to its package adapter; override here before install. Deeper per-adapter model config is editable on the agent after install." })}
          </li>
        </PreviewSection>
      )}
      ...
      {/* Env inputs */}
      {envInputs.length > 0 && (
        <PreviewSection title={t("teamCatalog.secretsEnvInputsCount", { defaultValue: "Secrets & env inputs · {{count}}", count: envInputs.length })}>
          {envInputs.map((input) => {
              ...
              <li key={formKey} className="grid gap-2 px-3 py-2 text-sm sm:grid-cols-[minmax(0,1fr)_minmax(14rem,18rem)] sm:items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <Input
                    ...
                    placeholder={input.requirement === "required" ? t("teamCatalog.requiredPlaceholder", { defaultValue: "Required" }) : t("teamCatalog.optionalPlaceholder", { defaultValue: "Optional" })}
                    aria-label={t("teamCatalog.envValueAria", { defaultValue: "{{key}} value", key: input.key })}
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        aria-label={visible ? t("teamCatalog.hideKey", { defaultValue: "Hide {{key}}", key: input.key }) : t("teamCatalog.showKey", { defaultValue: "Show {{key}}", key: input.key })}
                      >
                        {visible ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>{visible ? t("teamCatalog.hideValue", { defaultValue: "Hide value" }) : t("teamCatalog.showValue", { defaultValue: "Show value" })}</TooltipContent>
                  </Tooltip>
                </div>
              </li>
            );
      ...
      <div className="rounded-md border border-border px-3 py-2.5 text-xs text-muted-foreground">
        {t("teamCatalog.provenancePrefix", { defaultValue: "Imported entities are stamped with" })}{" "}
        <code className="font-mono">metadata.paperclip.catalogTeam</code>{" "}
        ({team.packageName ?? team.key}, {t("teamCatalog.contentHash", { defaultValue: "content hash" })} <code className="font-mono">{team.contentHash.slice(0, 16)}…</code>),{" "}
        {t("teamCatalog.provenanceSuffix", { defaultValue: "and an activity event is recorded for preview and install." })}
      </div>
    </div>
  );
}
...
export function ApplyProgress({ team }: { team: CatalogTeam }) {
  ...
  return (
    <div className="flex items-center gap-3 py-10 text-sm">
      ...
      <div>
        <p className="font-medium">{t("teamCatalog.installingNamed", { defaultValue: "Installing {{name}}…", name: team.name })}</p>
        <p className="text-xs text-muted-foreground">
          {t("teamCatalog.installingHint", { defaultValue: "Resolving skills, importing agents, projects, and routines. This may take a moment." })}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      <p className="text-sm text-muted-foreground">
        {t("teamCatalog.importedSummary", { defaultValue: "{{name}} was imported into your company. Imported entities are stamped with catalog provenance.", name: team.name })}
      </p>
      {result && (
      ...
      <div className="flex items-center gap-2">
        ...
        {outOfDate && (
          <Tooltip>
            ...
            <TooltipContent>{t("teamCatalog.updateAvailableOutOfDate", { defaultValue: "Update available — installed team is out of date" })}</TooltipContent>
          </Tooltip>
        )}
        {risk !== "safe" && (
          <Tooltip>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          {team.counts.agents === 1
            ? t("teamCatalog.agentCount_one", { defaultValue: "{{count}} agent", count: team.counts.agents })
            : t("teamCatalog.agentCount_other", { defaultValue: "{{count}} agents", count: team.counts.agents })} ·{" "}
          {team.counts.projects === 1
            ? t("teamCatalog.projectCount_one", { defaultValue: "{{count}} project", count: team.counts.projects })
            : t("teamCatalog.projectCount_other", { defaultValue: "{{count}} projects", count: team.counts.projects })} ·{" "}
          {team.counts.routines === 1
            ? t("teamCatalog.routineCount_one", { defaultValue: "{{count}} routine", count: team.counts.routines })
            : t("teamCatalog.routineCount_other", { defaultValue: "{{count}} routines", count: team.counts.routines })}
        </p>
      </div>
...
export function TeamCatalog() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("teamCatalog.orgChart", { defaultValue: "Org Chart" }), href: "/org" },
      { label: t("teamCatalog.teams", { defaultValue: "Teams" }), href: TEAM_CATALOG_ROUTE_ROOT },
    ]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border px-5 py-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-8">
              ...
              {kindFilter === "all" ? t("teamCatalog.allKinds", { defaultValue: "All kinds" }) : kindFilter === "bundled" ? t("teamCatalog.bundled", { defaultValue: "Bundled" }) : t("teamCatalog.optional", { defaultValue: "Optional" })}
        ...
        {categories.length > 0 && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-8">
                {categoryFilter ? t("teamCatalog.categoryNamed", { defaultValue: "Category · {{name}}", name: titleCase(categoryFilter) }) : t("teamCatalog.allCategories", { defaultValue: "All categories" })}
                <ChevronDown className="h-3.5 w-3.5" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-8">
              {riskFilter === "any" ? t("teamCatalog.anyRisk", { defaultValue: "Any risk" }) : riskFilter === "safe" ? t("teamCatalog.safeOnly", { defaultValue: "Safe only" }) : riskFilter === "has_warnings" ? t("teamCatalog.hasWarnings", { defaultValue: "Has warnings" }) : t("teamCatalog.blocked", { defaultValue: "Blocked" })}
              <ChevronDown className="h-3.5 w-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start">
            ...
            {anyFilterActive && (
              <>
                ...
                >
                  <RotateCcw className="h-3 w-3" /> {t("teamCatalog.resetFilters", { defaultValue: "Reset filters" })}
                </button>
              </>
            )}
          </DropdownMenuContent>
        ...
        {anyFilterActive && (
          <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => setSearchParams(new URLSearchParams())}>
            {t("teamCatalog.resetFilters", { defaultValue: "Reset filters" })}
          </Button>
        )}
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          {catalogQuery.isLoading ? (
            ...
            <div className="p-4">
              <div role="alert" className="rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2.5 text-sm text-rose-700 dark:text-rose-300">
                {t("teamCatalog.failedToLoad", { defaultValue: "Failed to load team catalog." })}
              </div>
              <Button variant="outline" size="sm" className="mt-3" onClick={() => catalogQuery.refetch()}>
                <RotateCcw className="h-3.5 w-3.5" /> {t("common.retry", { defaultValue: "Retry" })}
              </Button>
            </div>
          ) : teams.length === 0 ? (
            <EmptyState icon={Users2} message={t("teamCatalog.noCatalogConfigured", { defaultValue: "No team catalog configured." })} />
          ) : filtered.length === 0 ? (
            <EmptyState
              icon={Search}
              message={t("teamCatalog.noTeamsMatch", { defaultValue: "No teams match this filter." })}
            ...
            <div>
              {grouped.bundled.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.bundledCount", { defaultValue: "Bundled · {{count}}", count: grouped.bundled.length })}
                  </div>
              ...
              {grouped.optional.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.optionalCount", { defaultValue: "Optional · {{count}}", count: grouped.optional.length })}
                  </div>
              ...
              {grouped.installed.length > 0 && (
                <>
                  <div className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                    {t("teamCatalog.installedCount", { defaultValue: "Installed · {{count}}", count: grouped.installed.length })}
                  </div>
        ...
        {(isDesktop || selectedTeam) && (
          ...
          >
            {!isDesktop && selectedTeam && (
              ...
              >
                <ChevronLeft className="h-4 w-4" /> {t("teamCatalog.backToCatalog", { defaultValue: "Back to catalog" })}
              </button>
            )}
            {selectedTeam ? (
              <TeamDetailPane
              ...
              <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
                {t("teamCatalog.selectTeam", { defaultValue: "Select a team to view details." })}
              </div>
            )}
          </div>
        )}
      ...
      {selectedTeam && installOpen && (
        <TeamInstallerDialog
          ...
          onInstalled={() => {
            pushToast({ tone: "success", title: t("teamCatalog.teamInstalled", { defaultValue: "Team installed" }), body: t("teamCatalog.teamImportedToast", { defaultValue: "{{name}} was imported.", name: selectedTeam.name }) });
