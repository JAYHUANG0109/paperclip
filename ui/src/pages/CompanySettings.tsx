...
import { CloudUpload, Settings, Download, Upload } from "lucide-react";
...
export function CompanySettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("settings.company.breadcrumbCompany"), href: "/dashboard" },
      { label: t("settings.company.breadcrumbSettings") }
    ]);
  ...
  if (!selectedCompany) {
    return (
      <div className="text-sm text-muted-foreground">
        {t("settings.company.noCompanySelected")}
      </div>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <Field label={t("settings.company.companyNameLabel")} hint={t("settings.company.companyNameHint")}>
            <input
          ...
          <Field
            label={t("settings.company.descriptionLabel")}
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {logoUrl && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {clearLogoMutation.isPending ? t("settings.company.removingLogo") : t("settings.company.removeLogo")}
                      </Button>
                    </div>
                  )}
              ...
              <Field
                label={t("settings.company.brandColorLabel")}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {brandColor && (
                    ...
                    >
                      {t("settings.company.clear")}
                    </Button>
                  )}
                </div>
              </Field>
              <Field
                label={t("settings.company.attachmentLimitLabel")}
                hint={t("settings.company.attachmentLimitHint", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
              >
                <div className="flex flex-col gap-1.5">
                  ...
                  {!attachmentMaxValid && (
                    <span className="text-xs text-destructive">
                      {t("settings.company.attachmentLimitError", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
                    </span>
                  )}
                </div>
              </Field>
      ...
      {generalDirty && (
        <div className="flex items-center gap-2">
          ...
          >
            {generalMutation.isPending ? t("settings.company.saving") : t("settings.company.saveChanges")}
          </Button>
          {generalMutation.isSuccess && (
            <span className="text-xs text-muted-foreground">{t("settings.company.saved")}</span>
          )}
          {generalMutation.isError && (
            <span className="text-xs text-destructive">
              {generalMutation.error instanceof Error
                  ? generalMutation.error.message
                  : t("settings.company.failedToSave")}
            </span>
          )}
        </div>
      )}
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
          {t("settings.company.hiring")}
        </div>
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            label={t("settings.company.requireApprovalLabel")}
            hint={t("settings.company.requireApprovalHint")}
      ...
      <div className="space-y-4">
        <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
          {t("settings.company.companyPackages")}
        </div>
        <div className="rounded-md border border-border px-4 py-4">
          <p className="text-sm text-muted-foreground">
            {t("settings.company.packagesMovedPrefix")}{" "}
            <Link to="/org" className="underline hover:text-foreground">{t("settings.company.orgChart")}</Link> {t("settings.company.packagesMovedSuffix")}
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {cloudSyncEnabled ? (
              <Button size="sm" asChild>
                <Link to="/company/settings/cloud-upstream">
                  ...
                  {t("settings.company.sendToCloud")}
                </Link>
              </Button>
            ) : null}
            <Button size="sm" variant="outline" asChild>
              <Link to="/company/export">
                ...
                {t("settings.company.export")}
              </Link>
            </Button>
            <Button size="sm" variant="outline" asChild>
              <Link to="/company/import">
                <Upload className="mr-1.5 h-3.5 w-3.5" />
                {t("settings.company.import")}
              </Link>
            </Button>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="text-xs font-medium text-destructive uppercase tracking-wide">
          {t("settings.company.dangerZone")}
        </div>
        <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
          <p className="text-sm text-muted-foreground">
            {t("settings.company.archiveDescription")}
          </p>
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              ...
              onClick={() => {
                ...
                const confirmed = window.confirm(
                  t("settings.company.archiveConfirm", { name: selectedCompany.name })
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
            ...
            >
              {archiveMutation.isPending
                ? t("settings.company.archiving")
                : selectedCompany.status === "archived"
                ? t("settings.company.alreadyArchived")
                : t("settings.company.archiveCompany")}
            </Button>
            {archiveMutation.isError && (
              <span className="text-xs text-destructive">
                {archiveMutation.error instanceof Error
                  ? archiveMutation.error.message
                  : t("settings.company.failedToArchive")}
              </span>
            )}
          </div>
        </div>
