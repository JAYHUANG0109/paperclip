...
function scopeLabel(scope: CompanySearchScope): string {
  switch (scope) {
    ...
    case "artifacts": return t("search.scope.artifacts");
...
function subgroupLabel(key: SubGroupKey): string {
  switch (key) {
    ...
    case "artifacts": return t("search.subgroup.artifacts");
      ...
      <div className="flex items-center justify-between py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
        <span>
          {allMatchTotal > totalResults
            ? t("search.resultCountOfTotal", { count: totalResults, total: allMatchTotal })
            : totalResults === 1
              ? t("search.resultCount_one", { count: totalResults })
              : t("search.resultCount_other", { count: totalResults })}
          {` · ${t("search.sortedByField", { field: sortLabel })}`}
          {activeFilterCount > 0
