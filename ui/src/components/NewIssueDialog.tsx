...
const IssueDescriptionEditor = memo(function IssueDescriptionEditor({
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={t("newIssue.addDescription", { defaultValue: "Add description..." })}
      bordered={false}
      mentions={mentions}
...
export function NewIssueDialog() {
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("newIssue.claudeOptions", { defaultValue: "Claude options" })
      : assigneeAdapterType === "codex_local"
        ? t("newIssue.codexOptions", { defaultValue: "Codex options" })
        : assigneeAdapterType === "opencode_local"
          ? t("newIssue.opencodeOptions", { defaultValue: "OpenCode options" })
        : t("newIssue.agentOptions", { defaultValue: "Agent options" });
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("newIssue.createFailed", { defaultValue: "Failed to create task. Try again." });
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("newIssue.newSubTask", { defaultValue: "New sub-task" }) : t("newIssue.newTask", { defaultValue: "New task" })}</span>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <InlineEntitySelector
                ...
                disablePortal
                noneLabel={t("newIssue.noAssignee", { defaultValue: "No assignee" })}
                searchPlaceholder={t("newIssue.searchAssignees", { defaultValue: "Search assignees..." })}
                emptyMessage={t("newIssue.noAssigneesFound", { defaultValue: "No assignees found." })}
                onChange={(value) => {
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssue.assignee", { defaultValue: "Assignee" })}</span>
                  )
                }
                renderOption={(option) => {
              ...
              <InlineEntitySelector
                ...
                disablePortal
                noneLabel={t("newIssue.noProject", { defaultValue: "No project" })}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("newIssue.project", { defaultValue: "Project" })}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={taskWatchdogsEnabled ? t("newIssue.addReviewerApproverWatchdog", { defaultValue: "Add reviewer, approver, or watchdog" }) : t("newIssue.addReviewerApprover", { defaultValue: "Add reviewer or approver" })}
                  >
                ...
                <PopoverContent className="w-44 p-1" align="start">
                  ...
                  {taskWatchdogsEnabled && (
                    ...
                    >
                      ...
                      {t("newIssue.watchdog", { defaultValue: "Watchdog" })}
                    </button>
                  )}
                </PopoverContent>
              </Popover>
                ...
                <Popover open={watchdogEditorOpen} onOpenChange={setWatchdogEditorOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      title={t("newIssue.configureWatchdog", { defaultValue: "Configure watchdog" })}
                    >
                      {selectedWatchdogAgent ? (
                        ...
                        <span className="text-muted-foreground">{t("newIssue.setWatchdog", { defaultValue: "Set watchdog" })}</span>
                      )}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80 p-3 space-y-3" align="start">
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("newIssue.watchdogAgent", { defaultValue: "Watchdog agent" })}</div>
                      <InlineEntitySelector
                        value={watchdogAgentId}
                        options={watchdogAgentOptions}
                        placeholder={t("newIssue.selectAgent", { defaultValue: "Select agent" })}
                        noneLabel={t("newIssue.noWatchdogAgent", { defaultValue: "No watchdog agent" })}
                        searchPlaceholder={t("newIssue.searchAgents", { defaultValue: "Search agents..." })}
                        emptyMessage={t("newIssue.noAgentsFound", { defaultValue: "No agents found." })}
                        ...
                        renderTriggerValue={(option) =>
                          option ? (
                            ...
                            <span className="text-muted-foreground">{t("newIssue.selectAgent", { defaultValue: "Select agent" })}</span>
                          )
                        }
                        renderOption={(option) => {
                    ...
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("newIssue.instructions", { defaultValue: "Instructions" })} <span className="font-normal text-muted-foreground">{t("newIssue.optionalParen", { defaultValue: "(optional)" })}</span></div>
                      <Textarea
                        value={watchdogInstructions}
                        onChange={(event) => setWatchdogInstructions(event.target.value)}
                        placeholder={t("newIssue.watchdogInstructionsPlaceholder", { defaultValue: "What should the watchdog watch for and how should it keep work moving?" })}
                        rows={4}
                        className="text-xs"
                      />
                    </div>
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("newIssue.subTaskOf", { defaultValue: "Sub-task of" })}</span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("newIssue.executionWorkspace", { defaultValue: "Execution workspace" })}</div>
              <div className="text-[11px] text-muted-foreground">
                {t("newIssue.executionWorkspaceHint", { defaultValue: "Control whether this task runs in the shared workspace, a new isolated workspace, or an existing one." })}
              </div>
              <select
              ...
              >
                {EXECUTION_WORKSPACE_MODES.map((option) => (
                  <option key={option.value} value={option.value}>
                    {t(`newIssue.executionWorkspaceMode.${option.value}`, { defaultValue: option.label })}
                  </option>
                ))}
              </select>
              {executionWorkspaceMode === "reuse_existing" && (
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("newIssue.reusingWorkspace", { name: selectedReusableExecutionWorkspace.name, source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("newIssue.existingExecutionWorkspace", { defaultValue: "existing execution workspace" }), defaultValue: "Reusing {{name}} from {{source}}." })}
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {t("newIssue.parentWorkspaceWarning", { suffix: parentExecutionWorkspaceLabel ? ` (${parentExecutionWorkspaceLabel})` : "", defaultValue: "Warning: this sub-task will no longer use the parent task workspace{{suffix}}." })}
                </div>
              ) : null}
            </div>
            </div>
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("newIssue.modelLane", { defaultValue: "Model lane" })}</div>
                  <div
                    className="flex w-full overflow-hidden rounded-md border border-border"
                    role="radiogroup"
                    aria-label={t("newIssue.modelLane", { defaultValue: "Model lane" })}
                  >
                    {(["primary", ...(assigneeSupportsCheapLane ? (["cheap"] as const) : ([] as const)), "custom"] as const).map((lane) => (
                      ...
                      >
                        {lane === "primary"
                          ? t("newIssue.lane.primary", { defaultValue: "Primary" })
                          : lane === "cheap"
                            ? t("newIssue.lane.cheap", { defaultValue: "Cheap" })
                            : t("newIssue.lane.custom", { defaultValue: "Custom" })}
                      </button>
                    ))}
                  </div>
                  {assigneeModelLane === "cheap" && (
                  ...
                  {assigneeModelLane === "primary" && (
                    <p className="text-[11px] text-muted-foreground">{t("newIssue.lanePrimaryHint", { defaultValue: "Runs on the agent's primary model." })}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-[11px] text-muted-foreground">{t("newIssue.laneCustomHint", { defaultValue: "Override the model and effort for this task only." })}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssue.model", { defaultValue: "Model" })}</div>
                    <InlineEntitySelector
                      ...
                      searchPlaceholder={t("newIssue.searchModels", { defaultValue: "Search models..." })}
                      emptyMessage={t("newIssue.noModelsFound", { defaultValue: "No models found." })}
                ...
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">{t("newIssue.thinkingEffort", { defaultValue: "Thinking effort" })}</div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {thinkingEffortOptions.map((option) => (
                        ...
                        >
                          {t(`newIssue.thinkingEffortOption.${option.value || "default"}`, { defaultValue: option.label })}
                        </button>
                      ))}
                    </div>
                  </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("newIssue.assignedBacklogNote", { defaultValue: "Assigning implies executable intent — leave status as Backlog only to deliberately park this. The assignee will not be woken until status moves to Todo or In Progress." })}
            </span>
          </div>
        ) : null}
        ...
        {currentAssigneeLowTrust ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("newIssue.lowTrustNote", { defaultValue: "Low-trust review agent. It can only act inside its assigned review boundary; issue, project, or run policy defines the concrete scope." })}
            </span>
          </div>
        ) : null}
