...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: t("fileViewer.denialPolicyTitle", { defaultValue: "Viewer blocked for this file" }),
      body: t("fileViewer.denialPolicyBody", { defaultValue: "This file is not available through the viewer because it may contain sensitive data." }),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
      title: t("fileViewer.denialOutsideTitle", { defaultValue: "Path is outside the workspace" }),
      body: t("fileViewer.denialOutsideBody", { defaultValue: "The viewer can only open files that live under the issue's workspace." }),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />,
      title: t("fileViewer.denialArchivedTitle", { defaultValue: "Workspace is no longer available" }),
      body: t("fileViewer.denialArchivedBody", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  if (lower.includes("remote")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewer.denialRemoteTitle", { defaultValue: "Remote workspace preview not supported" }),
      body: t("fileViewer.denialRemoteBody", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewer.denialTooLargeTitle", { defaultValue: "File is too large to preview" }),
      body: t("fileViewer.denialTooLargeBody", { defaultValue: "This file exceeds the supported preview size." }),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("fileViewer.denialUnsupportedTitle", { defaultValue: "Preview not supported for this file type" }),
      body: t("fileViewer.denialUnsupportedBody", { defaultValue: "This file does not have a text, image, or video preview available." }),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: t("fileViewer.denialGenericTitle", { defaultValue: "Can't preview this file" }),
    body: fallback || t("fileViewer.denialGenericBody", { defaultValue: "The viewer was unable to load this file." }),
  };
}
        ...
        <>
          ...
          {state?.line ? (
            <>
              ...
              <span>
                {state.column
                  ? t("fileViewer.lineCol", { defaultValue: "Line {{line}}, Col {{column}}", line: state.line, column: state.column })
                  : t("fileViewer.line", { defaultValue: "Line {{line}}", line: state.line })}
              </span>
            </>
          ) : null}
        </>
      ) : state ? (
        <span className="h-3 w-28 rounded bg-muted animate-pulse" aria-label={t("fileViewer.loadingDetails", { defaultValue: "Loading file details" })} />
      ) : null}
    </div>
  );
}
...
export function FileContentViewer({ content, highlightedLine, onLoaded }: FileContentViewerProps) {
  ...
  useEffect(() => {
    if (!lines) return;
    onLoaded?.(t("fileViewer.fileLoadedLines", { defaultValue: "File loaded, {{count}} lines.", count: lines.length }));
  }, [lines, onLoaded, t]);
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewer.imageUnavailable", { defaultValue: "Image preview unavailable" })}
        />
      );
    }
    return (
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("fileViewer.videoUnavailable", { defaultValue: "Video preview unavailable" })}
        />
      );
    }
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-black p-4">
        <video
          ...
          preload="metadata"
          playsInline
          aria-label={t("fileViewer.videoPreviewLabel", { defaultValue: "Video preview: {{title}}", title: resource.title })}
  ...
  if (resource.previewKind === "unsupported" || !lines) {
    return (
      <FileViewerStateView
        icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
        title={t("fileViewer.denialUnsupportedTitle", { defaultValue: "Preview not supported for this file type" })}
        body={resource.contentType ? t("fileViewer.contentType", { defaultValue: "Content type: {{contentType}}", contentType: resource.contentType }) : undefined}
      />
    );
  }
  ...
  const rawSourceView = (
    <div
      ref={codeScrollRef}
      role="region"
      aria-label={t("fileViewer.sourceRegionLabel", { defaultValue: "{{title}} source", title: resource.title })}
      tabIndex={0}
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div className="absolute right-3 top-3 z-20">
        <div
          role="group"
          aria-label={t("fileViewer.markdownPreviewMode", { defaultValue: "Markdown preview mode" })}
        ...
        >
          <Button
            ...
            size="icon-sm"
            aria-label={t("fileViewer.showRenderedMarkdown", { defaultValue: "Show rendered Markdown" })}
            title={t("fileViewer.renderedMarkdown", { defaultValue: "Rendered Markdown" })}
          ...
          <Button
            ...
            size="icon-sm"
            aria-label={t("fileViewer.showRawMarkdown", { defaultValue: "Show raw Markdown" })}
            title={t("fileViewer.rawMarkdown", { defaultValue: "Raw Markdown" })}
      ...
      {markdownMode === "raw" ? (
        ...
        <div
          role="region"
          aria-label={t("fileViewer.renderedMarkdownRegionLabel", { defaultValue: "{{title}} rendered Markdown", title: resource.title })}
          tabIndex={0}
  ...
  if (elapsedMs < 400) {
    return (
      <div className="flex-1 space-y-2 p-6" aria-busy="true" aria-live="polite">
        <span className="sr-only">{t("fileViewer.loadingPreview", { defaultValue: "Loading file preview" })}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {t("fileViewer.loadingPreviewEllipsis", { defaultValue: "Loading file preview..." })}
      </div>
    </div>
  );
}
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewer.unableToLoad", { defaultValue: "Unable to load file." }));
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("fileViewer.unableToLoadContent", { defaultValue: "Unable to load file content." }));
    }
  ...
  const copyToClipboard = useCallback(async (value: string, field: "content" | "link", message: string) => {
    try {
      ...
      showCopyFeedback(null, t("fileViewer.copyFailed", { defaultValue: "Copy failed" }));
    } finally {
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      if (!content) {
        showCopyFeedback(null, t("fileViewer.contentsUnavailable", { defaultValue: "File contents unavailable" }));
        return;
      }
      const message = content.content.encoding === "base64"
        ? t("fileViewer.copiedFileData", { defaultValue: "Copied file data" })
        : t("fileViewer.copiedContents", { defaultValue: "Copied contents" });
  ...
  const handleCopyLink = useCallback(() => {
    ...
    void copyToClipboard(window.location.href, "link", t("fileViewer.copiedLink", { defaultValue: "Copied link" }));
  }, [copyToClipboard, t]);
  ...
  const title = state ? basename(state.path) : t("fileViewer.browseWorkspace", { defaultValue: "Browse workspace" });
  const description = state
    ? middleTruncatePath(state.path)
    : t("fileViewer.browseDescription", { defaultValue: "Search and preview files from this issue's workspace." });
            ...
            <div className="flex shrink-0 items-center gap-1 self-start">
              ...
              {cameFromBrowse ? (
                <Button
                  ...
                  aria-label={t("fileViewer.backToFiles", { defaultValue: "Back to files" })}
                >
                  <ArrowLeft className="h-3.5 w-3.5" />
                  {t("fileViewer.backToFiles", { defaultValue: "Back to files" })}
                </Button>
              ) : null}
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "content" ? t("fileViewer.copiedFileContents", { defaultValue: "Copied file contents" }) : t("fileViewer.copyFileContents", { defaultValue: "Copy file contents" })}
                  title={copiedField === "content" ? t("fileViewer.copiedContents", { defaultValue: "Copied contents" }) : t("fileViewer.copyFileContents", { defaultValue: "Copy file contents" })}
                  className="h-7 w-7"
                >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "link" ? t("fileViewer.copiedFileViewLink", { defaultValue: "Copied file view link" }) : t("fileViewer.copyFileViewLink", { defaultValue: "Copy link to this file view" })}
                  title={copiedField === "link" ? t("fileViewer.copiedLink", { defaultValue: "Copied link" }) : t("fileViewer.copyLink", { defaultValue: "Copy link" })}
                  className="h-7 w-7"
                >
                  {copyingField === "link" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
              ...
              <Button
                ...
                className="h-7 w-7"
                aria-label={t("fileViewer.closeViewer", { defaultValue: "Close file viewer" })}
          ...
          {state ? (
            <div className="flex min-h-0 flex-1 gap-3 bg-muted/30 p-3">
              ...
              <div
                ...
                aria-label={t("fileViewer.resizeFileTree", { defaultValue: "Resize file tree" })}
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("fileViewer.notFoundTitle", { defaultValue: "File not found" })}
          body={t("fileViewer.notFoundBody", { defaultValue: "That file was not found in the active workspace." })}
          actions={
            <>
              {onFallbackToProject ? (
                <Button type="button" variant="secondary" size="sm" onClick={onFallbackToProject}>
                  {t("fileViewer.tryProjectWorkspace", { defaultValue: "Try project workspace" })}
                </Button>
              ) : null}
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("fileViewer.noWorkspaceTitle", { defaultValue: "No workspace available" })}
          body={t("fileViewer.noWorkspaceBody", { defaultValue: "This issue does not have a workspace that supports preview yet." })}
        />
      );
    }
  ...
  if (resource.kind === "remote_resource") {
    return (
      <FileViewerStateView
        ...
        title={t("fileViewer.remoteComingSoonTitle", { defaultValue: "Remote workspace preview coming soon" })}
        body={t("fileViewer.remoteComingSoonBody", { defaultValue: "This workspace is hosted remotely; inline previews are not supported yet." })}
      />
    );
  }
