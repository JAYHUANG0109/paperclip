...
export function NewGoalDialog() {
  ...
  const levelLabels = useMemo<Record<string, string>>(
    () => ({
      company: t("goals.dialog.levelCompany"),
      team: t("goals.dialog.levelTeam"),
      agent: t("goals.dialog.levelAgent"),
      task: t("goals.dialog.levelTask"),
    }),
    [t],
  );
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("goals.dialog.noCompanySelected"));
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newGoalDefaults.parentId ? t("goals.dialog.headerSubGoal") : t("goals.dialog.headerGoal")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("goals.dialog.titlePlaceholder")}
            value={title}
        ...
        <div className="px-4 pb-2 overflow-y-auto max-h-[50vh]">
          <MarkdownEditor
            ...
            placeholder={t("goals.dialog.descriptionPlaceholder")}
            bordered={false}
          ...
          <Popover open={parentOpen} onOpenChange={setParentOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {currentParent ? currentParent.title : t("goals.dialog.parentGoal")}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-end px-4 py-2.5 border-t border-border">
          ...
          >
            {createGoal.isPending ? t("goals.dialog.creating") : newGoalDefaults.parentId ? t("goals.dialog.createSubGoal") : t("goals.dialog.createGoal")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
