...
export function ActiveAgentsPanel({
  ...
  const resolvedTitle = title ?? t("agentsPanel.title");
  const resolvedEmptyMessage = emptyMessage ?? t("agentsPanel.empty");
      ...
      {showMoreLink && hiddenRunCount > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {t("agentsPanel.moreRuns", { count: hiddenRunCount })}
          </Link>
        </div>
      )}
    </div>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("agentsPanel.liveNow") : run.finishedAt ? t("agentsPanel.finished", { time: relativeTime(run.finishedAt) }) : t("agentsPanel.started", { time: relativeTime(run.createdAt) })}</span>
            </div>
          </div>
