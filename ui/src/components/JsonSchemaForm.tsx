...
export function validateField(
  ...
  if (type === "string" || type === "secret-ref") {
    ...
    if (schema.minLength != null && str.length < schema.minLength) {
      return t("jsonSchemaForm.minLength", { count: schema.minLength });
    }
    if (schema.maxLength != null && str.length > schema.maxLength) {
      return t("jsonSchemaForm.maxLength", { count: schema.maxLength });
    }
    if (schema.pattern) {
  ...
  if (type === "number" || type === "integer") {
    ...
    if (schema.exclusiveMinimum != null && num <= schema.exclusiveMinimum) {
      return t("jsonSchemaForm.exclusiveMinimum", { value: schema.exclusiveMinimum });
    }
    if (schema.exclusiveMaximum != null && num >= schema.exclusiveMaximum) {
      return t("jsonSchemaForm.exclusiveMaximum", { value: schema.exclusiveMaximum });
    }
        ...
        <Textarea
          ...
          value={
            stringValue.length === 0
              ? ""
              : t("jsonSchemaForm.sensitiveHidden", { count: stringValue.length })
          }
          readOnly
          placeholder={String(defaultValue ?? "")}
          disabled={disabled}
    ...
    >
      <div className="space-y-2">
        <SecretBindingPicker
          ...
          emptyHint={t("jsonSchemaForm.noActiveSecrets")}
          disabled={disabled}
        />
        {!isBoundToSecret ? (
          showRawInput ? (
            ...
            <button
              ...
              disabled={disabled}
            >
              {t("jsonSchemaForm.orPasteRawValue")}
            </button>
          )
        ) : null}
      </div>
          ...
          >
            <div className="flex-1">
              <div className="mb-2 text-xs font-medium text-muted-foreground">
                {t("jsonSchemaForm.itemN", { n: index + 1 })}
              </div>
              <FormField
