...
function buildJobItem(
  ...
  return {
    ...
    serviceIndex: null,
    disabledReason: command.disabledReason ?? (!command.command ? t("workspaceRuntime.jobMissingCommand") : null),
  };
}
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: t("workspaceRuntime.untrackedServiceReason"),
    }));
    ...
    <div className={cn("flex gap-2", iconOnly ? "w-auto flex-row flex-wrap" : "w-full flex-col sm:w-auto sm:flex-row sm:flex-wrap")}>
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("workspaceRuntime.actionRun")
          : action === "start"
            ? t("workspaceRuntime.actionStart")
            : action === "stop"
              ? t("workspaceRuntime.actionStop")
              : t("workspaceRuntime.actionRestart");
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("workspaceRuntime.port", { port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  const { t } = useTranslation();
  const resolvedServiceEmptyMessageDefault = serviceEmptyMessage ?? t("workspaceRuntime.noServicesConfigured");
  const resolvedJobEmptyMessage = jobEmptyMessage ?? t("workspaceRuntime.noJobsConfigured");
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? resolvedServiceEmptyMessageDefault;
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className={cn("border border-border/70 bg-background p-3", square ? "rounded-none" : "rounded-xl")}>
        <div className="space-y-1">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspaceRuntime.workspaceCommands")}</div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? t("workspaceRuntime.servicesRunning", { count: runningCount }) : t("workspaceRuntime.noServicesRunning")}
            </span>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? t("workspaceRuntime.jobsAvailable", { count: resolvedSections.jobs.length })
                : t("workspaceRuntime.commandsIndependent")}
            </span>
          </div>
