    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("sidebarCompanyMenu.reorder", { name: company.name })}
    ...
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany ? t("sidebarCompanyMenu.openSwitcherNamed", { name: selectedCompany.name }) : t("sidebarCompanyMenu.openSwitcher")}
        >
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-[11px] font-semibold uppercase text-muted-foreground">
            {t("sidebarCompanyMenu.switchWorkspace")}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("sidebarCompanyMenu.noWorkspaces")}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany ? t("sidebarCompanyMenu.invitePeopleNamed", { name: selectedCompany.name }) : t("sidebarCompanyMenu.invitePeople")}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{t("sidebarCompanyMenu.companySettings")}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? t("sidebarCompanyMenu.signingOut") : t("sidebarCompanyMenu.signOut")}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
