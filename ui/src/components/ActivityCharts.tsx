...
function aggregateRuns(runs: readonly HeartbeatRun[] = []): DashboardRunActivityDay[] {
  ...
  for (const day of days) grouped.set(day, { date: day, succeeded: 0, failed: 0, recovered: 0, other: 0, total: 0, failedByErrorCode: {} });
  ...
  return (
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          const entry = grouped.get(day) ?? { date: day, succeeded: 0, failed: 0, recovered: 0, other: 0, total: 0, failedByErrorCode: {} };
          const total = entry.total;
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.runsTooltip", { day, count: total })}>
              {total > 0 ? (
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: statusLabel(s) }))} />
    </div>
  );
}
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.successRateTooltip", { day, percent: entry.total > 0 ? Math.round(rate * 100) : 0, succeeded: entry.succeeded, total: entry.total })}>
              {entry.total > 0 ? (
