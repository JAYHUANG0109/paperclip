...
// Error codes stay verbatim — they are identifiers an operator greps for, not prose.
function runDayTooltip(entry: DashboardRunActivityDay): string {
  const lines = [t("activityCharts.runsTooltip", { day: entry.date, count: entry.total })];
  if (entry.succeeded > 0) lines.push(`  ${t("activityCharts.tooltip.succeeded")}: ${entry.succeeded}`);
  if (entry.recovered > 0) {
    lines.push(`  ${t("activityCharts.tooltip.recovered")}: ${entry.recovered} (${t("activityCharts.tooltip.retrySucceeded")})`);
  }
  if (entry.failed > 0) {
    lines.push(`  ${t("activityCharts.tooltip.failed")}: ${entry.failed}`);
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(`  ${t("activityCharts.tooltip.other")}: ${entry.other}`);
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("activityCharts.legend.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("activityCharts.legend.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("activityCharts.legend.failed") },
    { color: runSegmentColors.other, label: t("activityCharts.legend.other") },
  ];
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.issuesTooltip", { day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`priority.${p}`) }))} />
    </div>
  );
}
...
function statusLabel(status: string): string {
  const labels: Record<string, string> = {
    todo: t("activityCharts.status.todo"),
    in_progress: t("activityCharts.status.in_progress"),
    in_review: t("activityCharts.status.in_review"),
    done: t("activityCharts.status.done"),
    blocked: t("activityCharts.status.blocked"),
    cancelled: t("activityCharts.status.cancelled"),
    backlog: t("activityCharts.status.backlog"),
  };
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: statusLabel(s) }))} />
    </div>
  );
}
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("activityCharts.successRateTooltip", { day, percent: entry.total > 0 ? Math.round(rate * 100) : 0, succeeded: effectiveSucceeded, total: entry.total })}>
              {entry.total > 0 ? (
