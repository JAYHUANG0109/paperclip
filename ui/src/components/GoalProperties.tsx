  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("goals.props.status")}>
          {onUpdate ? (
            <PickerButton
              current={goal.status}
        ...
        <PropertyRow label={t("goals.props.level")}>
          {onUpdate ? (
            <PickerButton
              current={goal.level}
              options={GOAL_LEVELS}
        ...
        <PropertyRow label={t("goals.props.owner")}>
          {ownerAgent ? (
            <Link
            ...
            <span className="text-sm text-muted-foreground">{t("goals.props.none")}</span>
          )}
        </PropertyRow>

        {goal.parentId && (
          <PropertyRow label={t("goals.props.parentGoal")}>
            <Link
      ...
      <div className="space-y-1">
        <PropertyRow label={t("goals.props.created")}>
          <span className="text-sm">{formatDate(goal.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("goals.props.updated")}>
