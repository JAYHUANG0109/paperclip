...
export function RoutineHistoryTab({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      pushToast({
        title: t("routineHistory.restoredToast", { from: restoredFromNumber, to: newNumber }),
        body: data.secretMaterials.length > 0
          ? t("routineHistory.restoredBodyWithSecrets")
          : t("routineHistory.restoredBody"),
        tone: "success",
      });
    ...
    onError: (error) => {
      pushToast({
        title: t("routineHistory.failedToRestore"),
        body: error instanceof Error ? error.message : t("routineHistory.couldNotRestore"),
        tone: "error",
      });
    },
  });
  ...
  if (revisionsQuery.error) {
    return (
      <div className="rounded-md border border-l-2 border-l-destructive border-border p-4 space-y-3">
        <div>
          <p className="text-sm font-medium">{t("routineHistory.couldNotLoad")}</p>
          <p className="text-xs text-muted-foreground">
            {revisionsQuery.error instanceof Error
              ? revisionsQuery.error.message
              : t("routineHistory.unknownLoadError")}
          </p>
        </div>
      ...
      <div className="space-y-4 min-w-0">
        ...
        {!isEditDirty && onlyBootstrapRevision ? (
          <div className="space-y-2">
            <EmptyState
              icon={HistoryIcon}
              message={t("routineHistory.noEditsYet")}
            />
            <p className="text-center text-xs text-muted-foreground">
              {t("routineHistory.noEditsHint")}
            </p>
          </div>
        ) : (
          selectedRevision && (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-200">
            {t("routineHistory.viewingRevision", { number: revisionNumber })}
          </p>
          <p className="text-xs text-muted-foreground">
            {t("routineHistory.viewingRevisionHint", { number: nextRevisionNumber })}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onReturn} disabled={pending}>
            {t("routineHistory.returnToCurrent")}
          </Button>
          <Button size="sm" onClick={onRestore} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {t("routineHistory.restoreAsNew")}
          </Button>
        </div>
      </div>
    </div>
...
function ConflictBanner({
  ...
  const labels = dirtyFields.length > 0
    ...
    : [t("routineHistory.theRoutine")];
  const fieldsText = formatDirtyFieldList(labels);
  return (
    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-medium text-amber-200">{t("routineHistory.unsavedEdits")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineHistory.unsavedEditsHint", { fields: fieldsText })}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={onDiscard}>
            {t("routineHistory.discardChanges")}
          </Button>
          <Button size="sm" onClick={onSave}>
            {t("routineHistory.saveAndContinue")}
          </Button>
        </div>
      </div>
    ...
    <aside className="space-y-1">
      <header className="flex items-center justify-between pb-2">
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistory.revisions")}
        </p>
        <span className="text-[11px] text-muted-foreground">{t("routineHistory.totalCount", { count: totalRevisions })}</span>
      </header>
      ...
      {totalRevisions > revisions.length && !showOlder && (
        <Button variant="ghost" size="sm" className="w-full" onClick={onShowOlder}>
          {t("routineHistory.showOlder", { count: totalRevisions - revisions.length })}
        </Button>
      )}
    </aside>
  );
...
function RevisionPreview({
  ...
  const restoreLabel = t("routineHistory.restoreThisRevision");
  ...
  const fieldRows: Array<{ key: string; label: string; value: string; differs: boolean }> = [
    ...
    {
      key: "projectId",
      label: t("routineHistory.field.project"),
    ...
    },
    {
      ...
      label: t("routineHistory.field.concurrency"),
    ...
    <div className="space-y-4">
      <header className={`${cardWrapper} p-4 space-y-2`}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1 min-w-0">
            <p className="text-sm font-medium">{t("routineHistory.rev", { number: revision.revisionNumber })}</p>
            <p className="text-xs text-muted-foreground truncate">
              {t("routineHistory.savedByActor", { time: relativeTime(revision.createdAt), actor: getActorLabel(revision) })}
              {revision.changeSummary ? ` · ${revision.changeSummary}` : ""}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" size="sm" onClick={onCompare}>
              ...
              {t("routineHistory.compareWithCurrent")}
            </Button>
            <Button
              size="sm"
              onClick={onRestore}
      ...
      <div className={`${cardWrapper} p-3`}>
        <p className="pb-2 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistory.structuredFields")}
        </p>
              ...
              <p className="text-sm">
                ...
                {row.differs && (
                  <span className="ml-2 rounded-full border border-amber-500/40 bg-amber-500/10 px-1.5 text-[10px] uppercase tracking-[0.12em] text-amber-200">
                    {t("routineHistory.differsFromCurrent")}
                  </span>
                )}
              </p>
            </div>
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
          {t("routineHistory.triggers", { count: triggers.length })}
        </p>
        {triggers.length === 0 ? (
          ...
          <ul className="divide-y divide-border">
            {triggers.map((trigger) => (
              <li key={trigger.id} className="py-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                >
                  {trigger.enabled ? t("routineHistory.enabled") : t("routineHistory.disabled")}
                </span>
              </li>
            ))}
          </ul>
      ...
      {snapshot.variables.length > 0 && (
        <div className={`${cardWrapper} p-3 space-y-2`}>
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
            {t("routineHistory.variables", { count: snapshot.variables.length })}
          </p>
          <ul className="divide-y divide-border">
            {snapshot.variables.map((variable) => (
              <li key={variable.name} className="py-2 flex items-center justify-between text-sm">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("routineHistory.variableDefault", { value: formatVariableDefault(variable) })}
                </span>
              </li>
            ))}
          </ul>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t("routineHistory.restoreDialogTitle", { number: target.revisionNumber })}</DialogTitle>
          <DialogDescription>
            {t("routineHistory.restoreDialogDesc", {
        ...
        <ul className="space-y-2 text-sm">
          <li className="flex items-start gap-2">
            ...
            {t("routineHistory.restoreEffectFields")}
          </li>
          {envDiffCounts.total > 0 && (
            <li className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t("routineHistory.restoreEffectSecrets", { counts: formatEnvDiffCounts(envDiffCounts) })}
            </li>
          )}
          <li className="flex items-start gap-2">
            <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-emerald-400" />
            {t("routineHistory.restoreEffectHistory")}
          </li>
          {recreatedWebhookLabels.map((label) => (
            <li key={label} className="flex items-start gap-2 text-amber-200">
              ...
              {t("routineHistory.restoreEffectWebhook", { label })}
            </li>
          ))}
        </ul>
        <div className="space-y-1.5">
          <Label htmlFor="restore-change-summary" className="text-xs">
            {t("routineHistory.changeSummaryLabel")}
          </Label>
          <Input
        ...
        <DialogFooter>
          ...
          <Button onClick={onConfirm} disabled={pending}>
            <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
            {pending ? t("routineHistory.restoring") : t("routineHistory.restoreAsRevision", { number: newRevisionNumber })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="!max-w-[90%] w-full max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>{t("routineHistory.compareTitle")}</DialogTitle>
        </DialogHeader>
        ...
        <div className="overflow-auto flex-1 space-y-4">
              ...
              <table className="w-full text-sm border border-border rounded-md overflow-hidden">
                <thead>
                  <tr className="text-xs uppercase tracking-wide bg-muted/30 text-muted-foreground">
                    <th className="px-3 py-2 text-left">{t("routineHistory.colField")}</th>
                    <th className="px-3 py-2 text-left">{t("routineHistory.colOldValue")}</th>
                    <th className="px-3 py-2 text-left">{t("routineHistory.colNewValue")}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {t("routineHistory.descriptionDiff")}
            </p>
        ...
        <DialogFooter className="justify-between sm:justify-between">
          ...
          {leftIsHistorical && left && (
            <Button onClick={() => onRestore(left)}>
              <RotateCcw className="mr-1.5 h-3.5 w-3.5" />
              {t("routineHistory.restoreRevAsNew", { number: left.revisionNumber })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
...
function DiffTable({ rows }: { rows: DiffRow[] }) {
  const { t } = useTranslation();
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("routineHistory.noDescriptionEither")}</p>;
  }
  if (rows.every((row) => row.kind === "context")) {
    return <p className="text-sm text-muted-foreground">{t("routineHistory.descriptionsIdentical")}</p>;
  }
  ...
  return (
    <div className="rounded-md border border-border text-xs font-mono leading-6 overflow-hidden">
      <div className="grid grid-cols-[56px_56px_24px_minmax(0,1fr)] border-b border-border/60 bg-muted/30 px-3 py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        ...
        <span />
        <span>{t("routineHistory.content")}</span>
      </div>
      {rows.map((row, index) => (
        <div
...
function getActorLabel(revision: RoutineRevision): string {
  if (revision.createdByUserId) return t("routineHistory.actor.board");
  if (revision.createdByAgentId) return t("routineHistory.actor.agent");
  return t("routineHistory.actor.system");
}

function resolveAgentName(agentId: string | null, lookup: AgentLookup) {
  if (!agentId) return t("routineHistory.unassigned");
  return lookup.get(agentId)?.name ?? agentId;
}

function resolveProjectName(projectId: string | null, lookup: ProjectLookup) {
  if (!projectId) return t("routineHistory.noProject");
...
function computeFieldChanges(
  ...
  compareScalar("title", t("routineHistory.field.title"), oldRoutine.title, newRoutine.title);
  compareScalar("priority", t("routineHistory.field.priority"), oldRoutine.priority, newRoutine.priority);
  compareScalar(
    "assigneeAgentId",
    t("routineHistory.field.defaultAgent"),
    resolveAgentName(oldRoutine.assigneeAgentId, agents),
    resolveAgentName(newRoutine.assigneeAgentId, agents),
  );
  compareScalar(
    "projectId",
    t("routineHistory.field.project"),
    resolveProjectName(oldRoutine.projectId, projects),
    resolveProjectName(newRoutine.projectId, projects),
  );
  compareScalar("concurrencyPolicy", t("routineHistory.field.concurrency"), oldRoutine.concurrencyPolicy, newRoutine.concurrencyPolicy);
  compareScalar("catchUpPolicy", t("routineHistory.field.catchUp"), oldRoutine.catchUpPolicy, newRoutine.catchUpPolicy);
  compareScalar("status", t("routineHistory.field.status"), oldRoutine.status, newRoutine.status);
  if (JSON.stringify(oldRoutine.variables) !== JSON.stringify(newRoutine.variables)) {
    changes.push({
      field: t("routineHistory.field.variables"),
