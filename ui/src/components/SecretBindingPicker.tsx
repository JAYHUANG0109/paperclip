...
function describeSecret(secret: CompanySecret): string {
  ...
  if (secret.managedMode === "external_reference") {
    return standaloneT("secretBindingPicker.externalProvider", { provider });
  }
  return provider;
}
...
export function SecretBindingPicker({
  ...
  const label = labelProp ?? t("secretBindingPicker.label");
  const placeholder = placeholderProp ?? t("secretBindingPicker.placeholder");
  const emptyHint = emptyHintProp ?? t("secretBindingPicker.emptyHint");
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      setCreateError(error instanceof Error ? error.message : t("secretBindingPicker.createFailed"));
    },
  });

  const versionDisplay = (selector: SecretVersionSelector | undefined) => {
    if (selector === undefined || selector === VERSION_LATEST) return t("secretBindingPicker.latest");
    return `v${selector}`;
  };
    ...
    <div className={cn("space-y-1.5", className)}>
      ...
      <div className="flex items-center gap-1.5">
        <div className="relative flex-1">
          ...
          >
            <option value="">{secretsQuery.isPending ? t("common.loading") : placeholder}</option>
            {selectedMissing && value ? (
              <option value={value.secretId}>{t("secretBindingPicker.missingSecret", { id: value.secretId.slice(0, 8) })}</option>
            ) : null}
        ...
        {allowVersionSelector ? (
          <select
            ...
            aria-label={t("secretBindingPicker.version")}
          >
            <option value={VERSION_LATEST}>{t("secretBindingPicker.latest")}</option>
            {selectedSecret
        ...
        <Button
          ...
          aria-label={t("secretBindingPicker.createSecret")}
        >
      ...
      {selectedSecret ? (
        <p className={cn("text-[11px] text-muted-foreground", statusTone(selectedSecret.status))}>
          {selectedSecret.status !== "active" ? t("secretBindingPicker.statusPrefix", { status: selectedSecret.status }) : null}
          {t("secretBindingPicker.boundTo", { version: versionDisplay(value?.version), key: selectedSecret.key })}
        </p>
      ) : selectedMissing ? (
        <p className="text-[11px] text-destructive flex items-center gap-1">
          <AlertCircle className="h-3 w-3" />
          {t("secretBindingPicker.missingHint")}
        </p>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("secretBindingPicker.createTitle")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-name">{t("secretBindingPicker.name")}</label>
              <Input
                id="secret-name"
                value={createName}
            ...
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-value">{t("secretBindingPicker.value")}</label>
              <Textarea
                id="secret-value"
                value={createValue}
                onChange={(event) => setCreateValue(event.target.value)}
                rows={3}
                placeholder={t("secretBindingPicker.valuePlaceholder")}
                className="font-mono text-xs"
              />
              <p className="text-[11px] text-muted-foreground mt-1">
                {t("secretBindingPicker.valueHint")}
              </p>
            </div>
            <div>
              <label className="text-xs font-medium text-foreground/80" htmlFor="secret-description">{t("secretBindingPicker.description")}</label>
              <Input
                id="secret-description"
                value={createDescription}
                onChange={(event) => setCreateDescription(event.target.value)}
                placeholder={t("secretBindingPicker.descriptionPlaceholder")}
              />
            </div>
          ...
          <DialogFooter>
            ...
            >
              ...
              {t("secretBindingPicker.createAndBind")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
