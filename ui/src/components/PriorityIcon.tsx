...
export function PriorityIcon({ priority, onChange, className, showLabel }: PriorityIconProps) {
  ...
  const label = (p: string) => t(`priority.${p}`, { defaultValue: priorityConfig[p]?.label ?? p });
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{icon}<span className="text-sm">{label(priority)}</span></span> : icon;
