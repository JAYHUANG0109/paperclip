...
export function AssigneeChip({
  ...
  if (assignee.agentId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="agent">
        <span className="sr-only">{t("interruptHandoff.agentPrefix", { defaultValue: "Agent " })}</span>
  ...
  if (assignee.userId) {
    return (
      <span className={cn(CHIP_CLASS, className)} data-testid="handoff-assignee-chip" data-kind="user">
        <span className="sr-only">{t("interruptHandoff.userPrefix", { defaultValue: "User " })}</span>
  ...
  return (
    ...
    >
      <span className="sr-only">{t("interruptHandoff.noAssigneePrefix", { defaultValue: "No assignee — " })}</span>
      {t("interruptHandoff.unassigned", { defaultValue: "Unassigned" })}
    </span>
  );
}
...
export function HandoffWakeRow({
  ...
  return (
    <div
      ...
      data-kind={info.kind}
    >
      <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t("interruptHandoff.wake", { defaultValue: "Wake" })}</span>
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        {t("interruptHandoff.mentionCoach", { matchedText: candidate.matchedText, defaultValue: "Did you mean @{{matchedText}}? Plain text won't notify or assign an agent." })}
      </span>
      <button
        type="button"
        onClick={onInsert}
        ...
        aria-label={t("interruptHandoff.insertMentionAria", { name: agentDisplayName, defaultValue: "Insert mention for {{name}} into your comment" })}
      >
    ...
    >
      ...
      {summary.nothingLive ? (
        <p role="status" className="text-xs text-muted-foreground" data-testid="pause-nothing-live">
          {t("interruptHandoff.nothingLive", { defaultValue: "Nothing live to pause — no agent run is in flight or queued. This records a hold so new work won't start until you resume." })}
        </p>
      ) : null}
