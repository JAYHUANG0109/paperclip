  ...
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1 rounded-sm border border-emerald-500/50 bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-900 dark:text-emerald-100">
        ...
        {t("issueInteraction.planDecomp.completed", { defaultValue: "Completed" })}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1 rounded-sm border border-amber-500/50 bg-amber-500/10 px-2 py-0.5 text-[11px] font-medium text-amber-900 dark:text-amber-100">
      ...
      {t("issueInteraction.planDecomp.inFlight", { defaultValue: "In flight" })}
    </span>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">{t("issueInteraction.planDecomp.title", { defaultValue: "Plan decomposition" })}</h3>
        <span className="text-[11px] text-muted-foreground/80">
          {t("issueInteraction.planDecomp.revisionCount", { count: items.length, defaultValue: "{{count}} accepted plan revisions" })}
        </span>
      </div>
      ...
      <ul className="space-y-3">
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ?? t("issueInteraction.planDecomp.agentFallback", { defaultValue: "agent" })
            : null;
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("issueInteraction.planDecomp.revisionLabel", { revision: record.acceptedPlanRevisionNumber, defaultValue: "revision {{revision}}" })
              : t("issueInteraction.planDecomp.revisionLabel", { revision: record.acceptedPlanRevisionId.slice(0, 8), defaultValue: "revision {{revision}}" });
          const completedAt =
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("issueInteraction.planDecomp.planRevision", { revisionLabel, defaultValue: "Plan {{revisionLabel}}" })}
                </span>
                <span className="text-xs text-muted-foreground/70">·</span>
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  <GitBranch className="h-3 w-3 text-muted-foreground" />
                  {t("issueInteraction.planDecomp.childrenCreated", { created, requested, count: requested, defaultValue: "{{created}} of {{requested}} child tasks created" })}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("issueInteraction.planDecomp.idempotentTitle", { defaultValue: "Repeat attempts with this fingerprint reuse this record instead of creating new children" })}
                  >
                    <Repeat className="h-3 w-3" />
                    {t("issueInteraction.planDecomp.idempotentClaim", { defaultValue: "Idempotent claim" })}
                  </span>
                ) : null}
              </div>

              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground">
                {ownerName ? <span>{t("issueInteraction.planDecomp.owner", { ownerName, defaultValue: "Owner: {{ownerName}}" })}</span> : null}
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>{t("issueInteraction.planDecomp.started", { time: relativeTime(startedAt), defaultValue: "Started {{time}}" })}</span>
                ) : null}
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>{t("issueInteraction.planDecomp.completedAt", { time: relativeTime(completedAt), defaultValue: "Completed {{time}}" })}</span>
                ) : updatedAt ? (
                  <span title={formatDateTime(updatedAt)}>{t("issueInteraction.planDecomp.updated", { time: relativeTime(updatedAt), defaultValue: "Updated {{time}}" })}</span>
                ) : null}
                {issueIdentifier ? (
                  <Link
                  ...
                  >
                    {t("issueInteraction.planDecomp.planDocument", { defaultValue: "Plan document" })}
                  </Link>
                ) : null}
              </div>
