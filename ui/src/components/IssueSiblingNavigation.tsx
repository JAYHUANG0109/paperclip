...
export function IssueSiblingNavigation({ navigation, linkState }: IssueSiblingNavigationProps) {
  ...
  return (
    <nav
      aria-label={t("issueSiblingNav.navAria")}
...
function SiblingLink({
  ...
  const label = direction === "previous" ? t("common.previous") : t("common.next");
  const ariaDirection = direction === "previous" ? t("issueSiblingNav.previousAria") : t("issueSiblingNav.nextAria");
  ...
  return (
    <Link
      ...
      aria-label={t("issueSiblingNav.linkAria", { direction: ariaDirection, identifier, title: issue.title })}
      className={cn(
