...
function getProjectStatuses() {
  return [
    ...
    { value: "planned", label: t("projectProperties.statusPlanned") },
    { value: "in_progress", label: t("status.in_progress") },
    { value: "completed", label: t("projectProperties.statusCompleted") },
...
function ArchiveDangerZone({
  ...
  const actionLabel = isArchive
    ? t("projectProperties.archiveProject")
    : t("projectProperties.unarchiveProject");
      ...
      {archivePending ? (
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {isArchive
              ? t("projectProperties.archiveConfirm", { name: project.name })
              : t("projectProperties.unarchiveConfirm", { name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{actionLabel}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{actionLabel}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("projectProperties.repoUrlInvalid"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("projectProperties.clearLocalFolderConfirm")
        : t("projectProperties.deleteLocalFolderConfirm"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("projectProperties.clearRepoConfirm")
        : t("projectProperties.deleteRepoConfirm"),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("projectProperties.field.name")} state={fieldState("name")} />}>
          {onUpdate || onFieldUpdate ? (
            <DraftInput
              ...
              placeholder={t("projectProperties.namePlaceholder")}
            />
          ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.field.description")} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            <InlineEditor
              ...
              placeholder={t("projectProperties.descriptionPlaceholder")}
              multiline
            />
          ) : (
            <p className="text-sm text-muted-foreground">
        ...
        <PropertyRow label={<FieldLabel label={t("projectProperties.field.status")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("projectProperties.field.lead")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.field.goals")} state={fieldState("goals")} />}
          alignStart
        ...
        <PropertyRow
          label={<FieldLabel label={t("projectProperties.field.env")} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-muted-foreground">
              {t("projectProperties.envHint")}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectProperties.field.created")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("projectProperties.field.updated")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("projectProperties.field.targetDate")} state="idle" />}>
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-[11px] text-muted-foreground">
                {t("projectProperties.legacyWorkspaces")}
              </div>
            )}
            ...
            <div className="space-y-1.5 rounded-md border border-border p-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder={t("projectProperties.workspacePathPlaceholder")}
                />
                <ChoosePathButton />
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top">
                    {t("projectProperties.executionWorkspacesTooltip")}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t("projectProperties.enableIsolatedCheckoutsDesc")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <ToggleSwitch
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("projectProperties.newIssuesDefaultIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("projectProperties.newIssuesDefaultIsolatedDesc")}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("projectProperties.hostManagedImpl")} <span className="text-foreground">{t("projectProperties.gitWorktree")}</span>
                        </div>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {t("projectProperties.provisionTeardownHint")}
                        </p>
                      </div>
                    ) : null}
                  </div>
