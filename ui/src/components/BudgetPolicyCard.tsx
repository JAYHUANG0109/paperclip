...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? t("budgetPolicy.lifetimeBudget") : t("budgetPolicy.monthlyUtcBudget");
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicy.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budgetPolicy.percentOfLimit", { percent: summary.utilizationPercent }) : t("budgetPolicy.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budgetPolicy.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budgetPolicy.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${t("budgetPolicy.pauseReasonSuffix", { reason: summary.pauseReason })}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("budgetPolicy.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("budgetPolicy.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicy.pausedProject")
          : t("budgetPolicy.pausedScope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("common.saving") : summary.amount > 0 ? t("budgetPolicy.updateBudget") : t("budgetPolicy.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("budgetPolicy.statusPaused") : summary.status === "warning" ? t("budgetPolicy.statusWarning") : summary.status === "hard_stop" ? t("budgetPolicy.statusHardStop") : t("budgetPolicy.statusHealthy")}
          </div>
        </div>
