  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("financeTimelineCard.title")}</CardTitle>
        <CardDescription>{t("financeTimelineCard.description")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{emptyMessage ?? t("financeTimelineCard.empty")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.id}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("financeTimelineCard.invoice", { id: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("financeTimelineCard.region", { region: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("financeTimelineCard.tier", { tier: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">{t("financeTimelineCard.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))
