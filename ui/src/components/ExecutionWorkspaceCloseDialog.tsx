...
export function ExecutionWorkspaceCloseDialog({
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("executionWorkspaceClose.retriedToast") : t("executionWorkspaceClose.closedToast"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("executionWorkspaceClose.failedToClose"),
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("executionWorkspaceClose.descriptionBefore")} <span className="font-medium text-foreground">{workspaceName}</span> {t("executionWorkspaceClose.descriptionAfter")}
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("executionWorkspaceClose.checkingSafe")}
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : t("executionWorkspaceClose.failedInspect")}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("executionWorkspaceClose.stateBlocked")
                  : readiness.state === "ready_with_warnings"
                    ? t("executionWorkspaceClose.stateWarnings")
                    : t("executionWorkspaceClose.stateReady")}
              </div>
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? t("executionWorkspaceClose.detailShared")
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? t("executionWorkspaceClose.detailOwnCheckout")
                    : readiness.isProjectPrimaryWorkspace
                      ? t("executionWorkspaceClose.detailPrimary")
                      : t("executionWorkspaceClose.detailDisposable")}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.blockingIssues")}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.blockingReasons")}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.warnings")}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.gitStatus")}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      ...
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("executionWorkspaceClose.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("executionWorkspaceClose.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("executionWorkspaceClose.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("executionWorkspaceClose.mergedIntoBase")}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("executionWorkspaceClose.unknown") : readiness.git.isMergedIntoBase ? t("common.yes") : t("common.no")}</div>
                    </div>
                    <div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.otherLinkedIssues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("executionWorkspaceClose.attachedServices")}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("executionWorkspaceClose.noDetails")}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("executionWorkspaceClose.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("executionWorkspaceClose.cleanupFailedNote")}
              </div>
            ) : null}
