  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routineVariables.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineVariables.detectedHintPrefix")} `{"{{name}}"}` {t("routineVariables.detectedHintSuffix")}
          </p>
        </div>
...
const BUILTIN_VARIABLE_DOCS: BuiltinVariableDoc[] = [
  {
    name: "date",
    example: "2026-04-28",
    description: standaloneT("routineVariables.builtinDateDesc"),
  },
  {
    name: "timestamp",
    example: "April 28, 2026 at 12:17 PM UTC",
    description: standaloneT("routineVariables.builtinTimestampDesc"),
  },
];
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        <span>
          {t("routineVariables.hintPrefix")} `{"{{variable_name}}"}` {t("routineVariables.hintSuffix")}
        </span>
        <button
          type="button"
          ...
          aria-label={t("routineVariables.showHelp")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routineVariables.dialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("routineVariables.dialogDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routineVariables.customVariables")}
              </h3>
              <p className="text-muted-foreground">
                {t("routineVariables.customType")}{" "}
                <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs text-foreground">
                  {"{{variable_name}}"}
                </code>{" "}
                {t("routineVariables.customDescPre")}{" "}
                <span className="font-medium text-foreground">{t("routineVariables.variables")}</span>
                {t("routineVariables.customDescPost")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routineVariables.bulletNames")}</li>
                <li>{t("routineVariables.bulletType")}</li>
                <li>{t("routineVariables.bulletReuse")}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routineVariables.builtinVariables")}
              </h3>
              <p className="text-muted-foreground">
                {t("routineVariables.builtinDesc")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.placeholder")}</th>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.example")}</th>
                      <th className="px-3 py-2 font-medium">{t("routineVariables.description")}</th>
                    </tr>
                  </thead>
