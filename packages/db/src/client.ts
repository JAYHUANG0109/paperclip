...
/**
 * This fork's connection defaults, applied when the environment says nothing.
 *
 * Upstream's reader returns an EMPTY options object when no DATABASE_* var is
 * set, which falls through to postgres.js defaults: pool 10, idle timeout
 * disabled, connect timeout 30. This deployment previously hardcoded 40/30/10,
 * so adopting upstream verbatim would have quietly cut the pool to a quarter —
 * the pool size is the ceiling on concurrent request handling long before
 * Postgres' own max_connections is, and this instance runs on one Mac mini.
 *
 * Keep 40 comfortably under the server's max_connections (200) so agents, CLI
 * and backups can still get connections. Releasing idle connections stops a
 * burst pinning the pool at max forever.
 */
const FORK_DEFAULTS: Required<Pick<DatabaseClientOptions,
  "maxConnections" | "idleTimeoutSeconds" | "connectTimeoutSeconds">> = {
  maxConnections: 40,
...
/**
 * Legacy alias. PAPERCLIP_DB_POOL_MAX predates upstream's DATABASE_POOL_MAX and
 * is set on existing deployments, so it keeps working; the upstream name wins
 * when both are present.
 */
function forkPoolMaxOverride(env: NodeJS.ProcessEnv): number | undefined {
  return envPositiveInteger(env, "PAPERCLIP_DB_POOL_MAX");
}

export function databaseClientOptionsWithForkDefaults(
  env: NodeJS.ProcessEnv = process.env,
): DatabaseClientOptions {
  const fromEnv = databaseClientOptionsFromEnv(env);
  return {
    ...FORK_DEFAULTS,
    ...(forkPoolMaxOverride(env) !== undefined
      ? { maxConnections: forkPoolMaxOverride(env) }
      : {}),
    ...fromEnv,
  };
}
...
export function createDb(url: string, options?: DatabaseClientOptions) {
  // Fork defaults, not the bare env reader: see databaseClientOptionsWithForkDefaults.
  const resolved = options ?? databaseClientOptionsWithForkDefaults();
