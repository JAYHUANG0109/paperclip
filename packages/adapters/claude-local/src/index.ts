...
- claudeAccountConfigDirs (string, optional): a rotation pool of pre-authenticated Claude credential directories (one per account/subscription = one separate 5h/weekly quota pool), separated by newlines, commas, or semicolons; a leading ~ is expanded. Pre-authenticate each with: CLAUDE_CONFIG_DIR=/path/to/dir claude auth login — Claude Code namespaces its credential store per config dir, so pooled accounts do not overwrite one another. When set (and autoSwitchAccountOnQuota is on), rotation is automatic and driven by two signals. Reactive (always available): a run that comes back with a provider quota error cools that account down until its window resets, and the next heartbeat moves to the next account, wrapping so a reset account becomes reusable. Proactive (best-effort): before a run, each account's usage is read from the provider and an account at/over quotaSwitchThresholdPercent is skipped without spending a heartbeat on it — this read needs an unexpired access token, which an idle account often lacks, so an unreadable account is treated as unknown and left in place rather than rotated off. An account that fails with "please log in" is also skipped for 15 minutes, so dead credentials cannot park the pool on a broken account. If every pooled account is cooling down, Paperclip keeps running on the current one and logs a warning — it never signs an account out to recover, because that would permanently subtract a working credential from the pool. Applies to both execution engines (acp and cli) on local (non-remote) subscription runs. An explicit env.CLAUDE_CONFIG_DIR pins the run and disables rotation for that agent.
- quotaSwitchThresholdPercent (number, optional, default 95): usage percentage at which an account is considered full. With a pool, an account whose usage reads at/over this is skipped before a run; with no pool, it triggers the browser account-switch flow.
