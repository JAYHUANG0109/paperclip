...
import type {
  ...
  PluginRespondInteractionParams,
  ScopeKey,
  ToolRunContext,
  ToolResult,
  EventFilter,
    ...
    return {
      ...
      issues: {
        ...
        async createComment(
          ...
          body: string,
          companyId: string,
          options?: { authorAgentId?: string; actorUserId?: string; authorUserEmail?: string },
        ) {
          return callHost("issues.createComment", {
            ...
            actorUserId: options?.actorUserId,
            authorUserEmail: options?.authorUserEmail,
          });
        },
        ...
        // Single merged entry point: upstream's `action`/`actorUserId` and the
        // fork's `decision`/`responderEmail`/`answers` both travel in one params
        // object (see PluginRespondInteractionParams).
        async respondInteraction(params: PluginRespondInteractionParams) {
