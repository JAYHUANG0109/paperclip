...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- When `PAPERCLIP_MEMORY_DIR` is set, it holds durable facts about the person you work for. Read its `MEMORY.md` index first and open only the entries you actually need. To remember something new, write it through the memory API (`PUT /api/companies/{companyId}/users/{PAPERCLIP_MEMORY_USER_ID}/memories/{name}`) — files you edit in that directory are overwritten from the database and never persist.",
