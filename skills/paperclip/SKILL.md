...
**Step 6 — Understand context.** Prefer `GET /api/issues/{issueId}/heartbeat-context` first. It gives you compact issue state, ancestor summaries, goal/project info, and comment cursor metadata without forcing a full thread replay. If `project.instructions` is present, treat it as authoritative project-specific guidance for this task — follow it alongside your own instructions. If `issue.skillHints` is a non-empty array, the human scoped this task to those skills: use ONLY those equipped skills for this task and do not invoke your other skills, unless the task cannot be completed without one (say so if it can't).
...
**Deliver files ONLY by uploading them as artifacts — never create native Google Workspace files directly.** When the deliverable is a document/slides/spreadsheet, generate a real file on the runner (`.docx`/`.pptx`/`.xlsx`/`.pdf`/etc.) and upload it as the artifact above. Do **not** use a Google Drive connector/tool to create a native Google Doc/Slides/Sheet as the deliverable: files created that way land in the user's My Drive **root**, bypass the "Paperclip 產出檔案" folder, and are not tracked by Paperclip. The upload path is the only one that reliably places the file in the user's output folder and links it on the task.
...
- **Making a NEW skill?** Create it as a MANAGED skill via `POST /api/companies/{companyId}/skills` with the full `markdown` and **`sharingScope: "private"`** — do NOT write a loose `SKILL.md` into `~/.claude/skills`. Creating it this way **auto-equips YOU (the calling agent)** and keeps it **private to you** (viewable + usable by you, not shared) until someone explicitly distributes it. Unmanaged local files can't be viewed, equipped, or distributed.
